...
class UjiSampelController extends Controller
{
    ...
    {
        dd("ini di multi rumus v2 ya dengan is multi print Y",$request->all());
        ...
        $request->validate([
            ...
            'analyses.*.No_Sementara' => 'nullable|string',
            'analyses.*.is_multi_print' => 'required|string',
            'analyses.*.No_Po_Multi_Sampel' => 'nullable|string', // Ini akan kita abaikan dan ambil dari log
            'analyses.*.Id_Resampling' => 'required|string', // Pastikan ini dikirim
            'analyses.*.parameters' => 'required|array|min:1',
            'analyses.*.parameters.*.Id_Quality_Control' => 'required',
            'analyses.*.parameters.*.Value_Parameter' => 'required|numeric',
        ], [
        ...
        try {
            ...
            if (!$userExists) {
                DB::rollBack(); // Rollback jika user tidak ada
                return response()->json([
                    'success' => false,
                    'status' => 404,
                    'message' => "User dengan ID $userId tidak ditemukan di sistem."
            ...
            $currentMonth = date('m', strtotime($dt)); // Gunakan tanggal server
            $currentYear = date('y', strtotime($dt)); // Gunakan tanggal server
            $prefix = 'FUS' . $currentMonth . $currentYear;
            $prefixLength = strlen($prefix);

            // Lock tabel untuk generate nomor faktur baru
            $lastNumber = DB::table('N_EMI_LAB_Uji_Sampel')
                ->where('No_Faktur', 'like', $prefix . '-%')
                ->lockForUpdate() 
                ->selectRaw("MAX(CAST(SUBSTRING(No_Faktur, ? + 2, 10) AS INT)) as max_number", [$prefixLength])
            ...
            $payloadActivityUjiSampel = [
                'Kode_Perusahaan' => '001',
                'No_Po_Sampel' => $firstAnalysis['No_Po_Sampel'],
                'No_Fak_Sub_Po' => null, // Kita belum tahu sub po yang benar di titik ini, mungkin diisi nanti
            ...
            foreach ($request->analyses as $index => $analysisData) {
                ...
                // 1. Decode Id_Resampling di awal loop
                try {
                ...
                // 2. Query log resampling untuk mendapatkan No_Fak_Sub_Po yang benar
                ...
                // 3. Simpan No_Fak_Sub_Po (No_Sampel_Resampling) yang benar dari database
                // Ini adalah nilai yang Anda inginkan, cth: 'FS1025-0803-2'
                $noFakSubPoYangBenar = $resamplingLog->No_Sampel_Resampling; 
                
                // Update log aktivitas utama dengan No_Fak_Sub_Po yang benar
                // (Asumsi hanya ada 1 resampling per request, jika >1, logikanya perlu disesuaikan)
                if ($index === 0) { // Hanya update di iterasi pertama
                ...
                $isFlagKhusus = DB::table('N_EMI_LAB_PO_Sampel')
                                    ->where('No_Sampel', $analysisData['No_Po_Sampel']) // Gunakan No_Po_Sampel dari request
                                    ->where('Flag_Khusus', 'Y')
                                    ->exists();
                ...
                if ($noSementara) {
                    ...
                    if ($dataSementara) {
                        ...
                        $sumberData = (object) [
                            'No_Po_Sampel' => $dataSementara->No_Po_Sampel,
                            'Id_Jenis_Analisa' => $jenisAnalisa,
                            'No_Po_Multi_Sampel' => $noFakSubPoYangBenar, // <-- GUNAKAN NILAI DARI LOG
                            'is_multi_print' => $dataSementara->Flag_Multi_QrCode,
                            'parameters' => $mergedParameters,
                            'formulas' => $analysisData['formulas'] ?? [],
                            "Id_Mesin" => $analysisData['id_mesin'],
                            "Tahapan_Ke" => $analysisData['Tahapan_Ke'],
                            "Id_Resampling" => $analysisData['Id_Resampling'] // Tetap simpan hashid-nya
                        ];
                    }
                } else {
                    // Blok jika BUKAN dari data sementara
                    $sumberData = (object) [
                        'No_Po_Sampel' => $analysisData['No_Po_Sampel'],
                        'Id_Jenis_Analisa' => $jenisAnalisa,
                        'No_Po_Multi_Sampel' => $noFakSubPoYangBenar, // <-- GUNAKAN NILAI DARI LOG
                ...
                // HAPUS BLOK DECODE Id_Resampling YANG LAMA DARI SINI
                // ... (BLOK LAMA SUDAH DIHAPUS) ...
                ...
                foreach ($formulas as $formula) {
                    ...
                    $calculatedResults[] = [
                        'No_Po_Sampel' => $sumberData->No_Po_Sampel,
                        'No_Sub_Sampel' => $sumberData->No_Po_Multi_Sampel, // <-- Sudah benar
                        'Id_Perhitungan' => $formula->id,
                        'Id_Jenis_Analisa' => $formula->Id_Jenis_Analisa,
                        'Hasil_Perhitungan' => $resultValue,
                        'Range_Awal' => $rangeAwal,
                ...
                foreach ($calculatedResults as $result) {
                    $RentangAwal = (float) $result['Range_Awal'];
                    $hasilFloat = $this->safeFloat($result['Hasil_Perhitungan']);
                    $Flag_Layak = ($hasilFloat < $RentangAwal) ? 'T' : 'Y'; // Logika Flag_Layak
                    ...
                    $payloadUjiSampleData[] = [
                        ...
                        "Flag_Perhitungan" => "Y",
                        "Flag_Multi_QrCode" => $sumberData->is_multi_print,
                        "No_Fak_Sub_Po" => $sumberData->No_Po_Multi_Sampel, // <-- Sudah benar
                        "Status" => null,
                        ...
                        "Range_Awal" => $result['Range_Awal'],
                        "Range_Akhir" => $result['Range_Akhir'],
                        "Flag_Resampling" => null, // Ini akan jadi data baru, jadi NULL
                        "Status_Keputusan_Sampel" => "menunggu",
                        'Flag_Layak' => $Flag_Layak,
                        "Flag_Final" => null,
                        'Id_Mesin' => $sumberData->Id_Mesin
                ...
                // Update log resampling MENJADI SELESAI
                DB::table("N_EMI_LAB_Uji_Sampel_Resampling_Log")
                    ->where('Id_Resampling', $id_resampling) // Gunakan ID numerik yang sudah di-decode
                    ->update([
            ...
            return response()->json([
                ...
                'status' => 500,
                'message' => "Terjadi Kesalahan Server: " . $e->getMessage(),
                'trace' => $e->getTraceAsString() // Hati-hati di produksi
            ], 500);
        }
    }
    ...
    public function storeMultiQrCodeNotRumusResampling(Request $request)
    {
        ...
        try {
            ...
            $fixedNoFakSubPo = $dataResampling->No_Sampel_Resampling; // contoh: FS1225-1155-2
            ...
            foreach ($request->analyses as $analysisData) {
                ...
                foreach ($sumberData->parameters as $parameter) {
                    ...
                    $payloadUjiSampleData[] = [
                        ...
                        "Hasil" => $paramValueFloat,
                        "Flag_Perhitungan" => null,
                        "Flag_Multi_QrCode" => $analysisData['is_multi_print'] ?? 'N', // Biasanya resampling 'Y' atau ikut settingan
                        "No_Fak_Sub_Po" => $fixedNoFakSubPo, // <--- INI YANG PENTING (DARI DB)
                        "Status" => null,
                        "Tanggal" => $tanggalSqlServer,
                        "Jam" => $jamSqlServer,
                        "Id_User" => $pengguna->UserId,
                        "No_Po_Sampel" => $firstAnalysis['No_Po_Sampel'],
                        'Status_Keputusan_Sampel' => 'menunggu',
                        'Tahapan_Ke' => $fixedTahapanKe, // <--- INI JUGA PENTING (DARI DB)
                        'Id_Mesin' => $analysisData['id_mesin'] ?? 3, // Default atau dari request
                        "Flag_String" => $flagString,
                        "Nilai_Hasil_String" => $nilaiHasilString
                    ];
                    ...
                    $payloadActivityUjiSampelHasil[] = [
                        ...
                        'Id_Log_Activity_Sampel' => $idLogActivity,
                        "No_Po_Sampel" => $firstAnalysis['No_Po_Sampel'],
                        "No_Fak_Sub_Po" => $fixedNoFakSubPo, // <--- DB
                        "Id_Jenis_Analisa" => $jenisAnalisaId,
                        "Value_Baru" => $paramValueFloat,
                        "Value_Lama" => $paramValueFloat,
                        "Tanggal" => $tanggalSqlServer,
                ...
                DB::table("N_EMI_LAB_Uji_Sampel_Resampling_Log")
                    ->where('Id_Resampling', $dataResampling->Id_Resampling) // Gunakan ID numerik yang sudah di-decode
                    ->update([
                        'Flag_Selesai_Resampling' => 'Y'
                    ]);
    ...
    public function storeMultiRumusNotMultiQrCode(Request $request)
    {
        ...
        dd("di multi rumus tapi tidak flag multi y", $request->all());
        ...
        foreach ($request->analyses as $analisis) {
            ...
            if ($checkFinishGood && $checkFinishGood->Flag_FG === 'Y') {
                ...
                if ($checkedPerhitungan->Flag_Perhitungan === 'Y') {
                    ...
                    // --- PERBAIKAN CORE LOGIC ---
                    // Hanya check Flag_Layak = 'T' pada data yang Status-nya masih NULL.
                    // Data lama yang Status-nya 'Y' (history) diabaikan.
                    $adaTidakLayak = DB::table('N_EMI_LAB_Uji_Sampel')
                        ->where('No_Po_Sampel', $analysis->No_Po_Sampel)
                        ->where('No_Fak_Sub_Po', $analysis->No_Fak_Sub_Po)
                        ->where('Id_Jenis_Analisa', $analysis->Id_Jenis_Analisa)
                        ->whereNull('Status') // <--- KUNCI: Hanya cek yang statusnya NULL (batch saat ini)
                        ->where('Flag_Layak', 'T') 
                        ->exists();

                    // Jika di batch inputan ini (Status NULL) ada yang T, maka T. Jika bersih, maka Y.
                    $statusKelayakan = $adaTidakLayak ? 'T' : 'Y';

                    DB::beginTransaction();

                    try {
                        // Update juga difilter berdasarkan Status yang NULL agar tidak mengupdate history
                        DB::table('N_EMI_LAB_Uji_Sampel')
                            ->where('No_Po_Sampel', $analysis->No_Po_Sampel)
                            ->where('No_Fak_Sub_Po', $analysis->No_Fak_Sub_Po)
                            ->where('Id_Jenis_Analisa', $analysis->Id_Jenis_Analisa)
                            ->whereNull('Status') // <--- Filter update juga pakai Status NULL
                            ->update([
                        ...
                        $payloadUjiFinalDetail = [
                            ...
                            'Flag_Layak' => $statusKelayakan, // Nilai sudah sesuai logic Status NULL
                            'Tanggal' => $tanggalSqlServer,
                            'Jam' => $jamSqlServer,
                            'Id_User' => $userId
                        ];
                    ...
                    // Logic Non-Perhitungan (Tetap pakai filter Status NULL untuk konsistensi)
                    DB::beginTransaction();
                    try {
                        DB::table('N_EMI_LAB_Uji_Sampel')
                            ->where('No_Po_Sampel', $analysis->No_Po_Sampel)
                            ->where('No_Fak_Sub_Po', $analysis->No_Fak_Sub_Po)
                            ->where('Id_Jenis_Analisa', $analysis->Id_Jenis_Analisa)
                            ->whereNull('Status') // Filter Status NULL
                            ->update([
                                'Status_Keputusan_Sampel' => 'terima',
                                'Flag_Selesai' => 'Y',
                                'Flag_Layak' => 'Y',
    ...
    public function getDataConfirmedSelesaiV2(Request $request)
    {
        ...
        // === Ambil semua data tambahan secara batch (OPTIMASI) ===
        $noPoSampelList = $paginatedData->pluck('No_Po_Sampel')->toArray();

        // 1. Query gabungan untuk info dan flag layak
        ...
        // 2. Proses di PHP untuk grouping
        $groupedInfos = $allInfoRows->groupBy('No_Po_Sampel');

        // 3. Buat Peta untuk info unik (PO, Mesin, dll)
        // Kita ambil item pertama dari tiap grup, karena data PO-nya pasti sama
        $uniqueInfos = $groupedInfos->map(function ($group) {
            return $group->first();
        });

        // 4. Ambil semua kode barang unik
        $kodeBarangList = $uniqueInfos->pluck('Kode_Barang')->unique()->filter()->toArray();
        $barangList = DB::table('N_EMI_View_Barang')
            ->whereIn('Kode_Barang', $kodeBarangList)
            ->pluck('Nama', 'Kode_Barang');

        // HAPUS QUERY $flagLayakList (sudah digabung)

        // === Transformasi tanpa N+1 Query ===
        $paginatedData->getCollection()->transform(function ($item) use ($uniqueInfos, $barangList, $groupedInfos) {
            $item->Id_Jenis_Analisa = Hashids::connection('custom')->encode($item->Id_Jenis_Analisa);

            // Gunakan $uniqueInfos untuk data PO
            $info = $uniqueInfos->get($item->No_Po_Sampel);

            if ($info) {
                // Ambil Flag_Layak dari item $baseQuery, BUKAN dari $info.
                // $info->Flag_Layak hanya mewakili 1 baris.
                // Kita akan proses Flag_Layak di bawah
                $item->Id_User    = $info->Id_User; // Note: Ini akan mengambil Id_User dari baris pertama
                $item->Jam        = $info->Jam; // Note: Ini akan mengambil Jam dari baris pertama
            ...
            // === [MODIFIKASI] Status Sampel ===
            $autoLolosKodes = ['PSZ'];
            $kodeAnalisa = trim($item->Kode_Analisa);

            if (in_array($kodeAnalisa, $autoLolosKodes)) {
                $item->Status_Sampel = "Lolos Uji";
            } elseif ($item->Flag_Multi_QrCode !== 'Y') {
                $item->Status_Sampel = "Lolos Uji";
            } else {
                // Gunakan $groupedInfos (pengganti $flagLayakList)
                $flags = $groupedInfos->get($item->No_Po_Sampel);

                if ($flags && $flags->isNotEmpty()) {
                    $flagsArray = $flags->pluck('Flag_Layak')->toArray();
                    
                    $allLolos = true;
                    if (empty($flagsArray)) $allLolos = false; 
            ...
            // Hapus Flag_Layak individual karena tidak relevan di level grup
  ...
  public function downloadRekapSampelByPdfV2(Request $request)
        ...
        foreach ($analysisIds as $index => $encodedId) {
            ...
            if ($currentAnalisa->Kode_Analisa === 'MBLG-STR') {
                ...
                $data = $query->select(
                    ...
                    ->groupBy('ps.No_Po', 'ps.No_Split_Po', 'ps.Kode_Barang', 'us.Id_Mesin') // <-- 'us.Tanggal' DIHAPUS
                    ->get();

                foreach ($data as $item) {
                    $key = $item->No_Split_Po . '|' . $item->Id_Mesin;
                    ...
                    if (!isset($results[$key])) {
                        ...
                        // MODIFIKASI: Kembalikan 'Tanggal_Produksi'
                        ...
                        $results[$key] = [
                            ...
                            'Tanggal_Produksi' => $tanggalProduksiFormatted, // <-- DIKEMBALIKAN
                            // 'Tanggal' => $this->formatTanggalIndoLengkap($item->Tanggal_Pengujian), // <-- TETAP DIHAPUS
                            'Analisa' => [],
                        ];
                    }
                ...
                $data = $query->select(
                    ...
                    ->groupBy('ps.No_Po', 'ps.No_Split_Po', 'ps.Kode_Barang', 'us.Id_Mesin', 'us.Flag_Perhitungan') // <-- 'us.Tanggal' DIHAPUS
                    ->get();
    ...
    public function downloadRekapSampelByExcellV2(Request $request)
    {
        ...
        foreach ($analysisIds as $encodedId) {
            ...
            $namaKolom = $currentAnalisa->Jenis_Analisa;
            $kodeAnalisa = $currentAnalisa->Kode_Analisa;
            ...
            if ($kodeAnalisa === 'MBLG-STR') {
                $query->where('us.Flag_Final', 'Y')->where('us.Status_Keputusan_Sampel', 'terima');
                ...
                $query->leftJoin('N_EMI_LAB_Perhitungan as p', 'p.id', '=', 'us.Id_Perhitungan');
            ...
            foreach($data as $item) {
                $item->meta_nama_kolom = $namaKolom;
                $item->meta_kode_analisa = $kodeAnalisa;
        ...
        $allKodeBarang = $allRawData->pluck('Kode_Barang')->unique()->filter()->values();
        $allIdMesin = $allRawData->pluck('Id_Mesin')->unique()->filter()->values();
        $allNoPo = $allRawData->pluck('No_Po')->unique()->filter()->values();
        ...
        foreach ($allRawData as $item) {
            ...
            if (!isset($results[$key])) {
                ...
                $namaSampel = $namaBarang . '-' . $item->No_Split_Po . '-' . $namaMesin;
                $tanggalProduksiFormatted = $tglProduksiRaw ? $this->formatTanggalIndoLengkap($tglProduksiRaw) : 'Tidak Ada';
            ...
            if ($item->meta_kode_analisa === 'MBLG-STR') {
                $nilaiUji = ($item->jumlah_tidak_layak > 0) ? 'Tidak lolos uji' : 'Lolos Uji';
                $results[$key]['Analisa'][] = ['nama' => $item->meta_nama_kolom, 'nilai' => $nilaiUji];
            } else {
                $results[$key]['Analisa'][] = [
                    'nama' => $item->meta_nama_kolom,
                    'nilai' => number_format((float)$item->Nilai_Rata_Rata, 2, '.', '')
                ];
            }
        }
