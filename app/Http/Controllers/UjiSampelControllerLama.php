...
class UjiSampelController extends Controller
{
        ...
        try {
            ...
            foreach ($request->analyses as $analysis) {
                ...
                $batchLogHasil[] = [
                    ...
                    "No_Po_Sampel" => $analysis['No_Po_Sampel'],
                    "Id_Jenis_Analisa" => $jenisAnalisa,
                    "Value_Baru" => $capturedResultValue,
                    "Value_Lama" => 0,
                    "Tanggal" => $tanggalSqlServer,
                    "Jam" => $jamSqlServer,
                    "Id_User" => $pengguna->UserId,
            ...
            if (!empty($batchUjiSementara)) {
                DB::table('N_EMI_LAB_Uji_Sampel_Sementara')->insert($batchUjiSementara);
            }
            
            if (!empty($batchDetailSementara)) {
                DB::table('N_EMI_LAB_Uji_Sampel_Detail_Sementara')->insert($batchDetailSementara);
            }
    ...
    {
        $Id_Jenis_Analisa = Hashids::connection('custom')->decode($Id_Jenis_Analisa)[0] ?? null;
            if (!$Id_Jenis_Analisa) {
                return response()->json(['success' => false, 'message' => 'ID Token tidak valid.'], 400);
            }
