...
class QuisyController extends Controller
{
    ...
    public function index()
    {
        ...
        // 2. Hitung PO yang selesai secara eksplisit MULAI DARI 16 Agustus 2025
        $explicitlyCompletedCount = DB::table('N_EMI_View_Order_Produksi')
            ...
            ->whereIn('No_Faktur', function ($query) {
                $query->select('No_Po')
                    ...
                    ->orWhere('Flag_Selesai', 'Y'); // Kondisi selesai atau ditutup
            })
            ->count();

        // 3. Jumlahkan keduanya untuk mendapatkan total PO yang selesai
        $completedPoCount = $implicitlyCompletedCount + $explicitlyCompletedCount;

        $uncompletedPoCount = DB::table('N_EMI_View_Order_Produksi')
            ->whereNull('status')
            ->where('Tanggal', '>=', '2025-08-16') // Opsi: Tambahkan ini agar konsisten dengan filter di fungsi get
            ->whereNotIn('No_Faktur', function ($query) {
                $query->select('No_Po')
                    ->from('N_EMI_LAB_PO_Sampel')
                    ->where('Flag_Close_Po', 'Y')
                    ; // Tambahkan kondisi OR di sini
            })
            ->count();
    ...
    public function closePoByProduksi(Request $request)
    {
        ...
        try {
            ...
            Log::error('Gagal menutup PO: ' . $e->getMessage());

            // 5. Kirim Respons Gagal (Server Error)
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan pada server saat mencoba menutup PO.'
    ...
    public function getMachinesByComputerKeys($computerKey, $noSplitPo, $noBatch)
    {
        ...
        // Sesuaikan urutan parameter dengan query
    ...
    //     $mesinList = DB::select($query, [$computerKey, $noSplitPo, $noBatch]);
    ...
    //             'result' => [], 'page' => 1, 
    //             'total_page' => 0, 'total_data' => 0, 'status_counts' => ['Terdaftar' => 0, 'Dibatalkan' => 0]], 200);
    //         }

    //         $statusCountsResult = $baseQuery->clone()->selectRaw("CASE WHEN pos.Status = 'Y' THEN 'Dibatalkan' ELSE 'Terdaftar' END as Status, COUNT(*) as total")->groupBy('pos.Status')->get();
    ...
    //           ->orderBy('pos.Tanggal', 'DESC')->orderBy('pos.Jam', 'DESC')->offset($offset)->limit($limit)->get();


    //         $noSampels = $mainData->pluck('No_Sampel')->unique()->all();
    //         $children = DB::table('N_EMI_LAB_PO_Sampel_Multi_QrCode')->whereIn('No_Po_Sampel', $noSampels)->get()->groupBy('No_Po_Sampel');

     
    //         $finalResult = $mainData->map(function ($item) use ($children) {
    //             $item->Id_Mesin = Hashids::connection('custom')->encode($item->Id_Mesin);
    //             $item->Berat_Sampel = (float) $item->Berat_Sampel; // Format jadi float 2 digit
    ...
    //     } catch (\Exception $e) {
    //         DB::log($e->getMessage());
    //         return response()->json(['success' => false, 'status' => 500, 'message' => ['error' => 'Terjadi Kesalahan']], 500);
    //     }
    // }
    ...
    public function cetakUlangQrCode(Request $request, $no_sampel, $id_mesin)
    {
        ...
        try {
            ...
            $batch = $getData->No_Batch ?? '-';
            $tanggal = date('d M Y');
            $namaMesin = $getInformasiMesin->Nama_Mesin ?? "Default";

            for ($i = 0; $i < $jumlahCetak; $i++) {
                $qrData = $getData->No_Sampel;
                $no_Split = $getData->No_Split_Po;

                $tsplContent  = "TEXT 30,20,\"3\",0,1,1,\"$getNamaSampel->Nama\"\r\n";
                $tsplContent .= "TEXT 31,20,\"3\",0,1,1,\"$getNamaSampel->Nama\"\r\n";
                $tsplContent .= "TEXT 30,90,\"2\",0,1,1,\"$qrData\"\r\n";
                $tsplContent .= "TEXT 30,140,\"2\",0,1,1,\"$no_Split\"\r\n";
                $tsplContent .= "TEXT 30,185,\"2\",0,1,1,\"Batch $batch\"\r\n";
                $tsplContent .= "TEXT 30,230,\"2\",0,1,1,\"$tanggal\"\r\n";
                $tsplContent .= "TEXT 450,70,\"1\",0,1,1,\"$namaMesin\"\r\n";
                $tsplContent .= "QRCODE 450,100,H,8,A,0,M2,S7,\"$qrData\"\r\n";
                
                $response = Http::timeout(360)->post('http://192.168.11.172:3000/print', [
                    'width'  => 100,
                    'height' => 50,
                    'gap'    => 3,
                    'data'   => $tsplContent
                ...
                if (!$response->successful()) {
                    return response()->json(['success' => false, 'status' => 500, 'message' => 'Gagal mengirim ke printer USB.'], 500);
                }
            }
            ...
            return response()->json([
                'success' => false,
                'status' => 500,
                'message' => 'Terjadi Kesalahan Saat Proses Cetak'
            ], 500);
        }
    }
