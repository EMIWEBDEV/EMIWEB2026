...
use Carbon\Carbon;
use ZipArchive;
use App\Jobs\LogLabActivityDetailsJob;
use App\Jobs\LogLabActivityDetailsJobCache;
...
class UjiSampelController extends Controller
{
    ...
    protected function calculateFormulaServerSideV2($formula, $parameterValues, $decimalPlaces = 2)
    {
        try {
            ...
            Log::channel('TrackingPerformersSementara')->error('Kesalahan kalkulasi rumus V2: ' . $e->getMessage(), [
                'formula_asli' => $formula,
                'formula_diproses_safe' => $safeFormula ?? $formula,
    ...
    public function storeMultiRumusV2(Request $request)
    {
        $request->validate([
            ...
            'analyses.*.No_Po_Multi_Sampel' => 'nullable|string',
            'analyses.*.parameters' => 'required|array|min:1',
            'analyses.*.formulas' => 'nullable|array',
            'analyses.*.id_mesin' => 'required|numeric',
        ], [
            'analyses.required' => 'Tidak ada data analisis yang dikirim.',
            'analyses.*.parameters.required' => 'Parameter tidak boleh kosong untuk setiap baris.',
        ...
        $logChannel = Log::channel('trackingPerformersToDatabaseMultiRumus'); 
        $startTime = microtime(true);
        $logChannel->info('--- [DEBUG] PROSES DIMULAI (v15 - Fixed Decimal Logic) ---');

        DB::beginTransaction();
        $logChannel->info('--- [DEBUG] 1. BeginTransaction Selesai', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

        try {
            $pengguna = Auth::user(); $userId = $pengguna->UserId;
            $prefix = 'FUS' . date('my');

            $logChannel->info('--- [DEBUG] 2. Memulai Kueri Setup Tergabung (Time, User, LastNumber Final)...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $setupSql = "
                SELECT 
                    (SELECT dbo.Get_Date_Time()) as DateTimeNow,
                    (SELECT 1 FROM N_EMI_LAB_Users WHERE UserId = ?) as UserExists,
                    (SELECT TOP 1 No_Faktur FROM N_EMI_LAB_Uji_Sampel WHERE No_Faktur LIKE ? ORDER BY No_Faktur DESC) as LastNoFaktur 
            "; 
            $setupResult = DB::selectOne($setupSql, [$userId, $prefix . '-%']);

            // 1. Proses Waktu Server
            if (empty($setupResult) || empty($setupResult->DateTimeNow)) {
                DB::rollBack(); throw new \Exception('Gagal mengambil waktu server.');
            }
            $dt = $setupResult->DateTimeNow;
            $tanggalSqlServer = date('Y-m-d', strtotime($dt));
            $jamSqlServer = date('H:i:s', strtotime($dt));

            // 2. Proses User Exists
            if (empty($setupResult->UserExists)) {
                DB::rollBack(); $logChannel->warning('User tidak ditemukan', ['userId' => $userId]);
                return response()->json(['success'=>false,'status'=>404,'message'=>'User tidak ditemukan'], 404);
            }

            // 3. Proses LastNumber
            $lastNumber = $setupResult->LastNoFaktur ? (int) substr($setupResult->LastNoFaktur, -4) : 0;
            $logChannel->info('--- [DEBUG] 3. Selesai Kueri Setup Tergabung', ['lastNumber' => $lastNumber, 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            // --- [AKHIR OPTIMISASI v13] ---

            // --- [OPTIMISASI v14] Pre-fetch Massal ---
            $prefetchStartTime = microtime(true);
            $logChannel->info('--- [DEBUG] 4. Memulai Pre-fetch Massal...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            // Kumpulkan ID unik dari request
            $allNoPoSampel = $request->collect('analyses')->pluck('No_Po_Sampel')->unique()->filter();
            $allJenisAnalisaHashes = $request->collect('analyses')->pluck('Id_Jenis_Analisa')->unique()->filter();
            $allNoSementara = $request->collect('analyses')->pluck('No_Sementara')->unique()->filter();
            $allMesinIds = $request->collect('analyses')->pluck('id_mesin')->unique()->filter();

            // Decode Hashids
            $allJenisAnalisaIds = $allJenisAnalisaHashes->map(function ($hash) {
                $decoded = Hashids::connection('custom')->decode($hash);
                return $decoded[0] ?? null;
            })->filter()->unique();

            // 1. Pre-fetch Otorisasi (2 kueri)
            $flagKhususMap = $allNoPoSampel->isNotEmpty() ?
                DB::table('N_EMI_LAB_PO_Sampel')->whereIn('No_Sampel', $allNoPoSampel)->where('Flag_Khusus', 'Y')->pluck('No_Sampel')->flip() : collect();

            $allowedAnalisaMap = $allJenisAnalisaIds->isNotEmpty() ?
                DB::table('N_EMI_LAB_Barang_Analisa')->where('Id_User', $userId)->whereIn('Id_Jenis_Analisa', $allJenisAnalisaIds)->pluck('Id_Jenis_Analisa')->flip() : collect();

            // 2. Pre-fetch Metadata (4 kueri)
            $kodeBarangMap = $allNoPoSampel->isNotEmpty() ?
                DB::table('N_EMI_LAB_PO_Sampel')->whereIn('No_Sampel', $allNoPoSampel)->pluck('Kode_Barang', 'No_Sampel') : collect();

            $formulaMap = $allJenisAnalisaIds->isNotEmpty() ?
                DB::table('N_EMI_LAB_Perhitungan')->whereIn('Id_Jenis_Analisa', $allJenisAnalisaIds)->get()->keyBy(function($item) {
                    return $item->Id_Jenis_Analisa . '-' . $item->id; // Key by JenisAnalisa-IdPerhitungan
                })->groupBy('Id_Jenis_Analisa') // Group by Jenis Analisa for easy lookup
                : collect();

            $mesinMap = $allMesinIds->isNotEmpty() ?
                DB::table('EMI_Master_Mesin')->whereIn('Id_Master_Mesin', $allMesinIds)->get()->keyBy('Id_Master_Mesin') : collect();

            // Pre-fetch Rentang (lebih kompleks, fetch berdasarkan key yang mungkin ada)
            $rentangKeys = new Collection();
            foreach ($request->analyses as $analysis) {
                 $po = $analysis['No_Po_Sampel'];
                 $jaDecoded = Hashids::connection('custom')->decode($analysis['Id_Jenis_Analisa']);
                 $jaId = $jaDecoded[0] ?? null;
                 $mesinId = $analysis['id_mesin'];
                 $kodeBarang = $kodeBarangMap->get($po);
                 if ($kodeBarang && $jaId && $mesinId) {
                     // Kita belum tahu ID Perhitungan, jadi fetch berdasarkan 3 kunci ini
                     $rentangKeys->push(['kb' => $kodeBarang, 'ja' => $jaId, 'ms' => $mesinId]);
                 }
            }
            $uniqueRentangKeys = $rentangKeys->unique(function($item){ return $item['kb'].'-'.$item['ja'].'-'.$item['ms']; });
            $rentangMap = collect();
            if ($uniqueRentangKeys->isNotEmpty()) {
                $rentangQuery = DB::table("N_EMI_LAB_Standar_Rentang");
                foreach($uniqueRentangKeys as $key) {
                    $rentangQuery->orWhere(function($q) use ($key){
                        $q->where('Kode_Barang', $key['kb'])
                          ->where('Id_Jenis_Analisa', $key['ja'])
                          ->where('Id_Master_Mesin', $key['ms']);
                    });
                }
                // Key by KodeBarang-JenisAnalisa-MesinId-IdPerhitungan
                $rentangMap = $rentangQuery->get()->keyBy(function($item) {
                     return $item->Kode_Barang . '-' . $item->Id_Jenis_Analisa . '-' . $item->Id_Master_Mesin . '-' . $item->Id_Perhitungan;
                });
            }

            // 3. Pre-fetch Data Sementara (2 kueri)
            $sementaraHeaderMap = collect();
            $sementaraDetailMap = collect(); // Akan berisi [No_Sementara => [Id_QC => Detail]]
            $sementaraDetailMapWithRV = collect(); // Akan berisi [No_Sementara => [No_Urut => RV_INT]]
            if ($allNoSementara->isNotEmpty()) {
                $sementaraHeaderMap = DB::table('N_EMI_LAB_Uji_Sampel_Sementara')
                                        ->whereIn('No_Sementara', $allNoSementara)
                                        ->get()
                                        ->keyBy('No_Sementara');

                $detailsSementara = DB::table('N_EMI_LAB_Uji_Sampel_Detail_Sementara')
                                        ->whereIn('No_Sementara', $allNoSementara)
                                        // Tambahkan kolom RV yang di-cast ke INT
                                        ->select('*', DB::raw('CAST(RV AS BIGINT) AS RV_INT'))
                                        ->get();

                // Grouping untuk lookup parameter dan RV check
                $sementaraDetailMap = $detailsSementara->groupBy('No_Sementara')->map(function ($items) {
                    return $items->keyBy('Id_Quality_Control'); // Key by QC ID for merging
                });
                $sementaraDetailMapWithRV = $detailsSementara->groupBy('No_Sementara')->map(function ($items) {
                    // Pastikan No_Urut bukan null sebelum keyBy
                    return $items->whereNotNull('No_Urut')->keyBy('No_Urut'); // Key by No_Urut for RV check
                });
            }

            $logChannel->info('--- [DEBUG] 4b. Selesai Pre-fetch Massal', ['elapsed' => number_format((microtime(true) - $prefetchStartTime) * 1000, 2) . ' ms']);
            // --- [AKHIR OPTIMISASI v14] ---


            // --- [OPTIMISASI v14] Early RV Check ---
            $logChannel->info('--- [DEBUG] 5. Memulai Early RV Check...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
           foreach ($request->analyses as $idx => $analysisData) {
                $noSementara = $analysisData['No_Sementara'] ?? null;
                if (!$noSementara || !$sementaraHeaderMap->has($noSementara)) {
                    continue; // Skip jika bukan dari sementara atau data sementara tidak ditemukan
                }

                $detailsSementaraForRV = $sementaraDetailMapWithRV->get($noSementara); // Map [No_Urut => Detail]
                if (!$detailsSementaraForRV) {
                    $logChannel->warning('RV Check: Detail sementara tidak ditemukan untuk header', ['no_sementara' => $noSementara]);
                    continue;
                }

                foreach ($analysisData['parameters'] as $paramFromRequest) {
                    // Decode No_Urut dari request
                    $idDecodedNu = Hashids::connection('custom')->decode($paramFromRequest['No_Urut'] ?? '');
                    $idNu = $idDecodedNu[0] ?? null; // ID No_Urut (numeric)

                    // Ambil RV_INT Hash dari request
                    $rvClientHash = $paramFromRequest['RV_INT'] ?? null;

                    // Hanya lakukan check jika No_Urut dan RV_INT Hash ada di request
                    if (is_null($idNu) || is_null($rvClientHash)) {
                        // Jika salah satu tidak ada, skip check untuk parameter ini
                        // Atau bisa juga throw error jika ini kondisi tidak valid
                        continue;
                    }

                    // Decode RV_INT Hash dari request
                    $rvDecodedClient = Hashids::connection('custom')->decode($rvClientHash);
                    $rvClientInt = $rvDecodedClient[0] ?? null; // RV dari client (numeric)

                    if (is_null($rvClientInt)) {
                        // Jika decode RV gagal, ini adalah data invalid dari client
                        DB::rollBack();
                        $logChannel->error('RV Check Gagal: Gagal decode RV_INT Hash dari client', ['no_sementara' => $noSementara, 'no_urut_hash' => $paramFromRequest['No_Urut'], 'rv_hash_client' => $rvClientHash]);
                        return response()->json(['success' => false, 'status' => 400, 'message' => 'Format RV_INT tidak valid.'], 400);
                    }

                    // Ambil detail parameter dari DB sementara (sudah di pre-fetch)
                    $dbDetailParam = $detailsSementaraForRV->get($idNu); // Cari berdasarkan No_Urut numeric

                    if (!$dbDetailParam) {
                        // Parameter dengan No_Urut ini tidak ada di DB sementara? Aneh.
                        DB::rollBack();
                        $logChannel->warning('RV Check Gagal: Detail param sementara tidak ditemukan di map', ['no_sementara' => $noSementara, 'no_urut_numeric' => $idNu]);
                        return response()->json(['success' => false, 'status' => 404, 'message' => 'Data parameter sementara tidak konsisten (No_Urut: ' . $paramFromRequest['No_Urut'] . ').'], 404);
                    }

                    // Bandingkan RV (pastikan keduanya integer)
                    // $dbDetailParam->RV_INT didapat dari CAST(RV AS BIGINT) saat pre-fetch
                    if ((int)$rvClientInt !== (int)$dbDetailParam->RV_INT) {
                        DB::rollBack();
                        $logChannel->warning('RV Check Gagal: Data Kedaluwarsa', ['no_sementara' => $noSementara, 'no_urut_numeric' => $idNu, 'rv_client_decoded' => $rvClientInt, 'rv_db' => $dbDetailParam->RV_INT]);
                        return response()->json(['success' => false, 'status' => 409, 'message' => 'Data sudah kedaluwarsa (No_Sementara: '.$noSementara.'). Silakan refresh halaman.'], 409);
                    }
                } // End loop parameter
           }
            $logChannel->info('--- [DEBUG] 5b. Selesai Early RV Check', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            // --- [AKHIR OPTIMISASI v14] ---


            // 3. Siapkan Array Bulk Operations & Counters (Tetap)
            $payloadsUjiSampel = []; // Untuk N_EMI_LAB_Uji_Sampel
            $payloadsUjiSampelDetail = []; // Untuk N_EMI_LAB_Uji_Sampel_Detail
            $payloadsActivityHasilDetail = []; // Untuk Log Hasil (Async)
            $payloadsActivityParameterDetail = []; // Untuk Log Parameter (Async)
            $sementaraToDelete = []; // Kumpulkan No_Sementara yang akan dihapus

            $totalAnalysisProcessed = count($request->analyses);
            $totalAnalysisBaru = 0; $totalAnalysisDariSementara = 0;
            $results = []; // Lupa diinisialisasi di v14


            // Insert log utama (Dipindahkan ke sini setelah pre-fetch)
            $firstAnalysis = $request->analyses[0];
            $idDecodedHeader = Hashids::connection('custom')->decode($firstAnalysis['Id_Jenis_Analisa']);
            $jenisAnalisaHeader = $idDecodedHeader[0] ?? null; // Tetap pakai yang sudah didecode

            $idLogActivity = DB::table('N_EMI_LAB_Activity_Uji_Sampel')->insertGetId([
                'Kode_Perusahaan' => '001', 'No_Po_Sampel' => $firstAnalysis['No_Po_Sampel'],
                'No_Fak_Sub_Po' => $firstAnalysis['No_Po_Multi_Sampel'], 'Jenis_Aktivitas' => 'save_submit',
                'Keterangan' => $pengguna->Nama . ' Berhasil Mengirimkan Data Analisa', 'Id_User' => $pengguna->UserId,
                'Tanggal' => $tanggalSqlServer, 'Jam' => $jamSqlServer, 'Id_Jenis_Analisa' => $jenisAnalisaHeader
            ], 'Id_Log_Activity');
            $logChannel->info('--- [DEBUG] 6. Insert Log Utama Selesai', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);


            // 4. Loop Utama (Sekarang hanya proses data, tanpa query DB)
            $logChannel->info('--- [DEBUG] 7. Mulai Loop Utama (Processing)', ['total_analyses' => $totalAnalysisProcessed, 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            foreach ($request->analyses as $idx => $analysisData) {

                $noSementara = $analysisData['No_Sementara'] ?? null;
                $currentNoPoSampel = $analysisData['No_Po_Sampel'];
                $idJenisAnalisaHash = $analysisData['Id_Jenis_Analisa'];
                $idMesin = $analysisData['id_mesin'];

                // Decode Jenis Analisa (lagi, tapi cepat dari memory)
                $idDecodedJA = Hashids::connection('custom')->decode($idJenisAnalisaHash);
                $jenisAnalisaId = $idDecodedJA[0] ?? null;

                if (!$jenisAnalisaId) {
                     DB::rollBack(); $logChannel->error('ID Jenis Analisa tidak valid', ['hash' => $idJenisAnalisaHash]);
                     return response()->json(['success' => false, 'status' => 400, 'message' => 'ID Jenis Analisa tidak valid.'], 400);
                }

                // Cek Otorisasi dari Map
                $isFlagKhusus = $flagKhususMap->has($currentNoPoSampel);
                if (!$isFlagKhusus) {
                    $isAllowed = $allowedAnalisaMap->has($jenisAnalisaId);
                    if (!$isAllowed) {
                        DB::rollBack(); $logChannel->warning('Akses ditolak', ['userId' => $userId, 'jenisAnalisaId' => $jenisAnalisaId]);
                        return response()->json(['success' => false, 'status' => 403, 'message' => 'Akses ditolak untuk jenis analisa ini.'], 403);
                    }
                }

                // Tentukan Sumber Data & User ID
                $sumberData = null;
                $idUserUntukInsert = $userId; // Default user saat ini
                $isFromSementara = false;

                // Decode parameter QC IDs dari request SEKARANG
                $decodedParametersRequest = collect($analysisData['parameters'])->mapWithKeys(function ($param) {
                    $decoded = Hashids::connection('custom')->decode($param['Id_Quality_Control']);
                    $qcId = $decoded[0] ?? null;
                    if ($qcId) {
                        // Simpan dengan key QC ID yang sudah didecode (numeric)
                        return [$qcId => [
                             'Value_Parameter' => $param['Value_Parameter'],
                             // Simpan info lain jika perlu (No_Urut, RV_INT) tapi Value_Parameter yg utama
                        ]];
                    }
                    return []; // Abaikan jika decode gagal
                })->filter(); // Hapus item kosong

                if ($noSementara && $sementaraHeaderMap->has($noSementara)) {
                    $dataSementara = $sementaraHeaderMap->get($noSementara);
                    $detailsSementara = $sementaraDetailMap->get($noSementara); // Ini adalah collection keyed by QC ID

                    if ($dataSementara && $detailsSementara) {
                        $isFromSementara = true;
                        $idUserUntukInsert = $dataSementara->Id_User; // Pakai user asli dari data sementara
                        $sementaraToDelete[] = $noSementara; // Tandai untuk dihapus nanti

                        // Gabungkan parameter: Ambil dari sementara, timpa Value_Parameter jika null & ada di request
                        $mergedParameters = $detailsSementara->map(function ($dbParam) use ($decodedParametersRequest) {
                            $qcId = $dbParam->Id_Quality_Control; // Sudah numeric
                            $finalValue = $dbParam->Value_Parameter;

                            // Jika nilai DB null DAN ada nilai baru dari request untuk QC ID ini
                            if (is_null($finalValue) && $decodedParametersRequest->has($qcId)) {
                                $finalValue = $decodedParametersRequest->get($qcId)['Value_Parameter'];
                            }

                            return [
                                'Id_Quality_Control' => $qcId, // Numeric
                                'Value_Parameter' => $finalValue
                            ];
                        })->values()->toArray(); // values() untuk reset keys jadi array biasa

                        $sumberData = (object) [
                            'No_Po_Sampel' => $dataSementara->No_Po_Sampel,
                            'Id_Jenis_Analisa' => $jenisAnalisaId, // Numeric
                            'No_Po_Multi_Sampel' => $dataSementara->No_Fak_Sub_Po,
                            'is_multi_print' => $dataSementara->Flag_Multi_QrCode ?? 'N',
                            'parameters' => $mergedParameters, // Parameter yang sudah digabung
                            
                            // ==================================
                            // === PERBAIKAN v15 - BAGIAN 1 ===
                            // ==================================
                            'formulas' => $analysisData['formulas'] ?? [], // <-- Tambahkan ini
                            
                            "Id_Mesin" => $idMesin // Ambil dari request asli
                        ];
                        $totalAnalysisDariSementara++;

                    } else {
                        // Header ada tapi detail tidak, atau sebaliknya? Error.
                        DB::rollBack(); $logChannel->error('Data sementara tidak konsisten', ['no_sementara' => $noSementara]);
                        return response()->json(['success' => false, 'status' => 500, 'message' => 'Data sementara tidak konsisten.'], 500);
                    }
                }

                // Jika bukan dari sementara ATAU data sementara tidak ditemukan
                if (!$sumberData) {
                    $sumberData = (object) [
                        'No_Po_Sampel' => $analysisData['No_Po_Sampel'],
                        'Id_Jenis_Analisa' => $jenisAnalisaId, // Numeric
                        'No_Po_Multi_Sampel' => $analysisData['No_Po_Multi_Sampel'],
                        'is_multi_print' => $analysisData['is_multi_print'] ?? 'N',
                        'parameters' => $decodedParametersRequest->map(function($paramData, $qcId) {
                            return ['Id_Quality_Control' => $qcId, 'Value_Parameter' => $paramData['Value_Parameter']];
                        })->values()->toArray(), // Ubah format ke array of objects
                        
                        // ==================================
                        // === PERBAIKAN v15 - BAGIAN 2 ===
                        // ==================================
                        'formulas' => $analysisData['formulas'] ?? [], // <-- Tambahkan ini
                ...
                $kodeBarang = $kodeBarangMap->get($sumberData->No_Po_Sampel);
                $formulasForJA = $formulaMap->get($sumberData->Id_Jenis_Analisa); // Ini collection of formulas
                $mesinData = $mesinMap->get($sumberData->Id_Mesin);

                if (!$kodeBarang) {
                    // Handle jika kode barang tidak ditemukan (seharusnya tidak terjadi jika data valid)
                    $logChannel->warning('Kode Barang tidak ditemukan untuk PO Sampel', ['no_po_sampel' => $sumberData->No_Po_Sampel]);
                    // Lanjutkan atau throw error? Tergantung requirement. Kita skip perhitungan rentang.
                }
                if (!$formulasForJA) {
                     // Handle jika tidak ada formula (seharusnya tidak terjadi)
                    $logChannel->warning('Formula tidak ditemukan untuk Jenis Analisa', ['id_jenis_analisa' => $sumberData->Id_Jenis_Analisa]);
                    $formulasForJA = collect(); // Kosongkan agar loop tidak error
                }


                // Siapkan parameter untuk kalkulasi (key harus numeric QC ID)
                $parameterValuesForCalc = collect($sumberData->parameters)->pluck('Value_Parameter', 'Id_Quality_Control');

                // ==========================================================
                // === PERBAIKAN v15 - BAGIAN 3 (LOGIKA INTI DARI V_LAMA) ===
                // ==========================================================

                // Siapkan formula dari request (client) untuk di-lookup
                // Ini adalah data $analysisData['formulas'] yang sudah kita sisipkan ke $sumberData
                $clientFormulaResults = collect($sumberData->formulas ?? [])->pluck('Hasil_Perhitungan', 'Rumus');


                // Lakukan Kalkulasi & Persiapan Payload Insert
                foreach ($formulasForJA as $formula) { // Loop formula yang relevan saja
                    $idPerhitungan = $formula->id;

                    // 1. Ambil rumus asli dari DB (e.g., [123] + [456])
                    $rawDbFormula = $formula->Rumus;

                    // 2. Ubah rumus DB menjadi format HASHID (e.g., [abc] + [def]) agar sama kuncinya dengan data client
                    // Ini adalah logika yang sama persis dari v_lama
                    $hashedDbFormula = $rawDbFormula;
                    preg_match_all('/\[(\d+)\]/', $hashedDbFormula, $matches);
                    if (!empty($matches[1])) {
                        // Gunakan array_unique untuk efisiensi
                        foreach (array_unique($matches[1]) as $numericId) {
                            $encoded = Hashids::connection('custom')->encode($numericId);
                            // str_replace aman di sini karena kita replace [id_angka]
                            $hashedDbFormula = str_replace("[$numericId]", "[$encoded]", $hashedDbFormula);
                        }
                    }

                    // 3. Cari hasil perhitungan dari client menggunakan rumus HASHID sebagai kunci
                    $hasilDariRequest = $clientFormulaResults->get($hashedDbFormula);
                    
                    $resultValue = null;

                    // 4. Tentukan nilai akhir: Gunakan hasil client JIKA ADA, jika tidak, baru hitung di server
                    if (!is_null($hasilDariRequest) && is_numeric($hasilDariRequest)) {
                        // Client mengirim hasil, PERCAYAI ITU.
                        // $this->safeFloat akan menanganinya
                        $resultValue = $hasilDariRequest;
                        
                    } else {
                        // Client tidak mengirim hasil, atau formatnya tidak valid, hitung di server.
                        // PENTING: Gunakan RUMUS ASLI DB ([123]), BUKAN yang di-hash ([abc])
                        
                        // Tentukan presisi. Gunakan dari DB, tapi jika 0 atau null, paksa ke 4 (atau 5)
                        $precision = $formula->Digit ?? 4; // Default 4 jika null
                        if ($precision == 0) {
                            $precision = 4; // Paksa 4 jika DB-nya 0
                        }

                        $resultValue = $this->calculateFormulaServerSideV2(
                            $rawDbFormula, // <-- Gunakan rumus asli
                            $parameterValuesForCalc,
                            $precision // <-- Gunakan presisi yang sudah diperbaiki
                        );
                    }

                    // $hasilFloat akan mengambil $resultValue (baik dari client atau server)
                    $hasilFloat = $this->safeFloat($resultValue);
                    
                    // --- AKHIR PERBAIKAN v15 ---


                    // Cari Rentang dari Map
                    $rentangKey = $kodeBarang . '-' . $sumberData->Id_Jenis_Analisa . '-' . $sumberData->Id_Mesin . '-' . $idPerhitungan;
                    $getDataRange = $rentangMap->get($rentangKey);
                    $rangeAwal = $getDataRange?->Range_Awal;
                    $rangeAkhir = $getDataRange?->Range_Akhir;

                    // Tentukan Flag Layak
                    $Flag_Layak = null;
                    if (!is_null($hasilFloat) && !is_null($rangeAwal)) {
                        $Flag_Layak = ($hasilFloat < (float)$rangeAwal) ? 'T' : 'Y';
                    }

                    // Tentukan apakah mesin FG
                    $isMesinFg = $mesinData && $mesinData->Flag_FG == 'Y';

                    // Buat Payload Uji Sampel (tabel utama)
                    // Hanya insert jika mesin FG atau jika tidak ada data mesin (fallback)
                    if ($isMesinFg || !$mesinData) {
                         $payloadsUjiSampel[] = [
                            "No_Faktur" => $newNumber, "Kode_Perusahaan" => "001",
                            "Id_Jenis_Analisa" => $sumberData->Id_Jenis_Analisa, // Numeric
                            "Id_Perhitungan" => $idPerhitungan,
                            "Hasil" => $hasilFloat, // <-- INI SEKARANG SUDAH BENAR (e.g., 379.1469)
                            "Flag_Perhitungan" => "Y",
                            "Flag_Multi_QrCode" => $sumberData->is_multi_print,
                            "No_Fak_Sub_Po" => $sumberData->No_Po_Multi_Sampel,
                            "Status" => null, 'Tahapan_Ke' => 1,
                            "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer,
                            "Id_User" => $idUserUntukInsert,
                            "No_Po_Sampel" => $sumberData->No_Po_Sampel,
                            "Range_Awal" => $rangeAwal, "Range_Akhir" => $rangeAkhir,
                            "Flag_Resampling" => null, "Status_Keputusan_Sampel" => "menunggu",
                            'Flag_Layak' => $Flag_Layak, "Flag_Final" => null,
                            'Id_Mesin' => $sumberData->Id_Mesin
                         ];
                    } else {
                         // Jika mesin tidak FG, kita tidak simpan hasil utamanya
                         // Tapi log tetap disimpan
                    }

                    // Buat Payload Log Hasil (Async)
                    $payloadsActivityHasilDetail[] = [
                        "Kode_Perusahaan" => "001", 'Id_Log_Activity_Sampel' => $idLogActivity,
                        "No_Po_Sampel" => $sumberData->No_Po_Sampel, "No_Fak_Sub_Po" => $sumberData->No_Po_Multi_Sampel,
                        "Id_Jenis_Analisa" => $sumberData->Id_Jenis_Analisa, // Numeric
                        "Id_Perhitungan" => $idPerhitungan,
                        "Value_Baru" => $hasilFloat, "Value_Lama" => $hasilFloat, // Untuk insert baru, value lama = value baru
                        "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer,
                        "Id_User" => $idUserUntukInsert, "Status_Submit" => "Submited",
                    ];
                } // End loop formula

                // Buat Payload Uji Sampel Detail & Log Parameter
                foreach ($sumberData->parameters as $parameter) {
                     $paramValueFloat = $this->safeFloat($parameter['Value_Parameter']);
                     $qcIdNumeric = $parameter['Id_Quality_Control']; // Sudah numeric

                    // Payload Detail (tabel utama)
                    $payloadsUjiSampelDetail[] = [
                        "Kode_Perusahaan" => "001", "No_Faktur_Uji_Sample" => $newNumber,
                        "Id_Quality_Control" => $qcIdNumeric, // Numeric
                        "Value_Parameter" => $paramValueFloat,
                        "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer,
                        "Id_User" => $idUserUntukInsert,
                    ];

                    // Payload Log Parameter (Async)
                    $payloadsActivityParameterDetail[] = [
                        "Kode_Perusahaan" => "001", 'Id_Log_Activity_Sampel' => $idLogActivity,
                        "No_Po_Sampel" => $sumberData->No_Po_Sampel, "No_Fak_Sub_Po" => $sumberData->No_Po_Multi_Sampel,
                        "Id_Jenis_Analisa" => $sumberData->Id_Jenis_Analisa, // Numeric
                        "Id_Quality_Control" => $qcIdNumeric, // Numeric
                        "Value_Baru" => $paramValueFloat, "Value_Lama" => $paramValueFloat, // Insert baru
                        "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer,
                        "Id_User" => $idUserUntukInsert, "Status_Submit" => "Submited",
                    ];
                } // End loop parameter
                ...
                $results[] = [
                    'generated_no_faktur' => $newNumber,
                    'status' => $isFromSementara ? 'migrated_from_temporary' : 'new_data',
                ];

            } // --- AKHIR LOOP UTAMA ---
            $logChannel->info('--- [DEBUG] 7b. Selesai Loop Utama (Processing)', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);


            // --- [OPTIMISASI v14] Eksekusi Bulk Operations ---
            $bulkOpsStartTime = microtime(true);
            $logChannel->info('--- [DEBUG] 8. Memulai Bulk Operations...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            // 1. Bulk Insert Data Utama
            if (!empty($payloadsUjiSampel)) {
                 // Chunk insert jika payload terlalu besar (misal > 500 baris)
                 foreach (array_chunk($payloadsUjiSampel, 500) as $chunk) {
                     DB::table('N_EMI_LAB_Uji_Sampel')->insert($chunk);
                 }
            }
            $logChannel->info('--- [DEBUG] 8a. Bulk Insert Uji Sampel Selesai', ['count' => count($payloadsUjiSampel), 'elapsed' => number_format((microtime(true) - $bulkOpsStartTime) * 1000, 2) . ' ms']);

            if (!empty($payloadsUjiSampelDetail)) {
                 foreach (array_chunk($payloadsUjiSampelDetail, 500) as $chunk) {
                    DB::table('N_EMI_LAB_Uji_Sampel_Detail')->insert($chunk);
                 }
            }
            $logChannel->info('--- [DEBUG] 8b. Bulk Insert Uji Sampel Detail Selesai', ['count' => count($payloadsUjiSampelDetail), 'elapsed' => number_format((microtime(true) - $bulkOpsStartTime) * 1000, 2) . ' ms']);

            // 2. Bulk Delete Data Sementara
            $uniqueSementaraToDelete = array_unique($sementaraToDelete);
            if (!empty($uniqueSementaraToDelete)) {
                DB::table('N_EMI_LAB_Uji_Sampel_Detail_Sementara')->whereIn('No_Sementara', $uniqueSementaraToDelete)->delete();
                DB::table('N_EMI_LAB_Uji_Sampel_Sementara')->whereIn('No_Sementara', $uniqueSementaraToDelete)->delete();
            }
            $logChannel->info('--- [DEBUG] 8c. Bulk Delete Sementara Selesai', ['count' => count($uniqueSementaraToDelete), 'elapsed' => number_format((microtime(true) - $bulkOpsStartTime) * 1000, 2) . ' ms']);

            $logChannel->info('--- [DEBUG] 8d. Selesai Bulk Operations', ['elapsed' => number_format((microtime(true) - $bulkOpsStartTime) * 1000, 2) . ' ms']);
            // --- [AKHIR OPTIMISASI v14] ---


            // Commit Transaksi DATA UTAMA
            $logChannel->info('--- [DEBUG] 9. Mulai Commit (Data Utama)', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            DB::commit();
            $logChannel->info('--- [DEBUG] 10. Commit (Data Utama) Selesai', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            // DISPATCH JOB SETELAH COMMIT BERHASIL (Untuk Log Detail)
            if (!empty($payloadsActivityHasilDetail) || !empty($payloadsActivityParameterDetail)) {
                LogLabActivityDetailsJob::dispatch($payloadsActivityHasilDetail, $payloadsActivityParameterDetail);
                $logChannel->info('--- [DEBUG] 11. Job LogLabActivityDetailsJob dispatched', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            }

            // LOGGING PERFORMA AKHIR
            $endTime = microtime(true);
            $executionTime = $endTime - $startTime;
            $totalRowsInsertedSynchronously = count($payloadsUjiSampel) + count($payloadsUjiSampelDetail);
            $totalLogRowsQueued = count($payloadsActivityHasilDetail) + count($payloadsActivityParameterDetail);
            $totalRowsDeleted = count($uniqueSementaraToDelete) * 2; // Estimasi kasar

            $logChannel->info('✅ PROSES STORE MULTI RUMUS FINAL SELESAI (v15 - Fixed)', [
                'status' => 'Optimal (Full Optimize: Index, Pre-fetch, Bulk Ops, Async Log, Fixed Decimal Logic)',
                'executionTime' => number_format($executionTime, 4) . ' detik',
                'userId' => $userId,
                'totalAnalysisInRequest' => $totalAnalysisProcessed,
                'totalAnalysisInsertedNew' => $totalAnalysisBaru,
                'totalAnalysisMigrated' => $totalAnalysisDariSementara,
                'totalRowsInsertedSynchronously' => $totalRowsInsertedSynchronously,
                'totalLogRowsQueued' => $totalLogRowsQueued,
                'totalTempRowsDeleted' => $totalRowsDeleted,
            ]);

            return response()->json([
                'success' => true, 'status' => 201,
                'message' => 'Data berhasil diproses dan disimpan (Log detail diproses di background).',
                'results' => $results
            ], 201);

        } catch (\Exception $e) {
            DB::rollBack();
            $logChannel->info('--- [DEBUG] Exception Terjadi, Rollback', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $endTime = microtime(true); $executionTime = isset($startTime) ? ($endTime - $startTime) : -1;

            $logChannel->error('❌ GAGAL STORE MULTI RUMUS FINAL (v15 - Fixed)', [
                'status' => 'Gagal (Exception)',
                'executionTime' => number_format($executionTime, 4) . ' detik',
                'userId' => $pengguna->UserId ?? 'unknown',
                'errorMessage' => $e->getMessage(),
                'errorLine' => $e->getLine(),
                'errorFile' => $e->getFile(),
                'requestPayloadCount' => $request->collect('analyses')->count()
            ]);
            return response()->json(['success' => false, 'status' => 500, 'message' => 'Terjadi Kesalahan Server: ' . $e->getMessage()], 500);
        } finally {
            $logChannel->info('--- [DEBUG] PROSES SELESAI (finally)', ['total_elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
        }
    }
    ...
    public function storeMultiRumusResamplingV2(Request $request)
    {
        $request->validate([
            ...
            'analyses.*.Tahapan_Ke' => 'required|numeric', 
        ], [
            'analyses.required' => 'Tidak ada data analisis yang dikirim.',
            'analyses.*.parameters.required' => 'Parameter tidak boleh kosong untuk setiap baris.',
            'analyses.*.Id_Resampling.required' => 'Id_Resampling wajib ada.',
            'analyses.*.Tahapan_Ke.required' => 'Tahapan_Ke wajib ada.',
        ]);

        $logChannel = Log::channel('trackingPerformersToDatabaseResampling');
        $startTime = microtime(true);
        $logChannel->info('--- [DEBUG] PROSES RESAMPLING DIMULAI (vOpt.2) ---');
        ...
        try {
            ...
            $logChannel->info('--- [DEBUG] 2. Memulai Kueri Setup Tergabung...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $setupSql = "
                SELECT 
                    (SELECT dbo.Get_Date_Time()) as DateTimeNow,
                    (SELECT 1 FROM N_EMI_LAB_Users WHERE UserId = ?) as UserExists,
            ...
            if (empty($setupResult) || empty($setupResult->DateTimeNow)) {
                DB::rollBack();
                throw new \Exception('Gagal mengambil waktu server.');
            }
            $dt = $setupResult->DateTimeNow;
            $tanggalSqlServer = date('Y-m-d', strtotime($dt));
            $jamSqlServer = date('H:i:s', strtotime($dt));
            ...
            if (empty($setupResult->UserExists)) {
                DB::rollBack();
                $logChannel->warning('User tidak ditemukan', ['userId' => $userId]);
                return response()->json(['success' => false, 'status' => 404, 'message' => 'User tidak ditemukan'], 404);
            }
            ...
            // 4. Pre-fetch Massal (Gaya v14/v15)
            $prefetchStartTime = microtime(true);
            $logChannel->info('--- [DEBUG] 4. Memulai Pre-fetch Massal...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            ...
            $formulaMap = $allJenisAnalisaIds->isNotEmpty() ?
                DB::table('N_EMI_LAB_Perhitungan')->whereIn('Id_Jenis_Analisa', $allJenisAnalisaIds)->get()->groupBy('Id_Jenis_Analisa') : collect();

            $mesinMap = $allMesinIds->isNotEmpty() ?
                DB::table('EMI_Master_Mesin')->whereIn('Id_Master_Mesin', $allMesinIds)->get()->keyBy('Id_Master_Mesin') : collect();

            // Pre-fetch Rentang (kompleks, tapi efisien)
            $rentangKeys = new Collection();
            foreach ($request->analyses as $analysis) {
                $po = $analysis['No_Po_Sampel'];
                $jaDecoded = Hashids::connection('custom')->decode($analysis['Id_Jenis_Analisa']);
            ...
            $uniqueRentangKeys = $rentangKeys->unique(function ($item) {
                return $item['kb'] . '-' . $item['ja'] . '-' . $item['ms'];
            });
            $rentangMap = collect();
            if ($uniqueRentangKeys->isNotEmpty()) {
                $rentangQuery = DB::table("N_EMI_LAB_Standar_Rentang");
            ...
            // 3. Pre-fetch Data Sementara (2 kueri) - Termasuk RV_INT untuk validasi
            $sementaraHeaderMap = collect();
            $sementaraDetailMap = collect(); // [No_Sementara => [Id_QC => Detail]]
            $sementaraDetailMapWithRV = collect(); // [No_Sementara => [No_Urut => Detail]]
            if ($allNoSementara->isNotEmpty()) {
                $sementaraHeaderMap = DB::table('N_EMI_LAB_Uji_Sampel_Sementara')
                    ->whereIn('No_Sementara', $allNoSementara)
                    ->get()
                ...
                $detailsSementara = DB::table('N_EMI_LAB_Uji_Sampel_Detail_Sementara')
                    ->whereIn('No_Sementara', $allNoSementara)
                    ->select('*', DB::raw('CAST(RV AS BIGINT) AS RV_INT')) // Ambil RV_INT
                    ->get();

                $sementaraDetailMap = $detailsSementara->groupBy('No_Sementara')->map(function ($items) {
                    return $items->keyBy('Id_Quality_Control');
                });
                $sementaraDetailMapWithRV = $detailsSementara->groupBy('No_Sementara')->map(function ($items) {
                    return $items->whereNotNull('No_Urut')->keyBy('No_Urut');
                });
            }

            $existingResamplingMap = $allNoPoSampel->isNotEmpty() ?
                DB::table('N_EMI_LAB_Uji_Sampel')
                    ->whereIn('No_Po_Sampel', $allNoPoSampel)
                    ...
                    ->map(function ($items) {
                        return $items->pluck('No_Fak_Sub_Po')->flip(); 
                    }) : collect();

            $logChannel->info('--- [DEBUG] 4b. Selesai Pre-fetch Massal', ['elapsed' => number_format((microtime(true) - $prefetchStartTime) * 1000, 2) . ' ms']);

            $logChannel->info('--- [DEBUG] 5. Memulai Early RV Check (Resampling)...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            foreach ($request->analyses as $idx => $analysisData) {
                $noSementara = $analysisData['No_Sementara'] ?? null;
                if (!$noSementara || !$sementaraHeaderMap->has($noSementara)) {
                    continue; // Skip jika bukan dari sementara
                }

                $detailsSementaraForRV = $sementaraDetailMapWithRV->get($noSementara);
                if (!$detailsSementaraForRV) {
                    continue;
                }
                ...
                foreach ($analysisData['parameters'] as $paramFromRequest) {
                    ...
                    $rvDecodedClient = Hashids::connection('custom')->decode($rvClientHash);
                    $rvClientInt = $rvDecodedClient[0] ?? null;

                    if (is_null($rvClientInt)) {
                        DB::rollBack();
                        $logChannel->error('RV Check Gagal: Gagal decode RV_INT Hash', ['no_sementara' => $noSementara, 'rv_hash_client' => $rvClientHash]);
                        return response()->json(['success' => false, 'status' => 400, 'message' => 'Format RV_INT tidak valid.'], 400);
                    }

                    $dbDetailParam = $detailsSementaraForRV->get($idNu);

                    if (!$dbDetailParam) {
                        DB::rollBack();
                        $logChannel->warning('RV Check Gagal: Detail param sementara tidak ditemukan', ['no_sementara' => $noSementara, 'no_urut_numeric' => $idNu]);
                        return response()->json(['success' => false, 'status' => 404, 'message' => 'Data parameter sementara tidak konsisten (No_Urut: ' . $paramFromRequest['No_Urut'] . ').'], 404);
                    }

                    if ((int)$rvClientInt !== (int)$dbDetailParam->RV_INT) {
                        DB::rollBack();
                        $logChannel->warning('RV Check Gagal: Data Kedaluwarsa', ['no_sementara' => $noSementara, 'rv_client_decoded' => $rvClientInt, 'rv_db' => $dbDetailParam->RV_INT]);
                        return response()->json(['success' => false, 'status' => 409, 'message' => 'Data sudah kedaluwarsa (No_Sementara: ' . $noSementara . '). Silakan refresh halaman.'], 409);
                    }
                }
            }
            ...
            $payloadsActivityParameterDetail = [];
            $sementaraToDelete = [];
            $resamplingIdsToUpdate = []; 
            $results = [];


            $firstAnalysis = $request->analyses[0];
            $idDecodedHeader = Hashids::connection('custom')->decode($firstAnalysis['Id_Jenis_Analisa']);
            $jenisAnalisaHeader = $idDecodedHeader[0] ?? null;

            $idLogActivity = DB::table('N_EMI_LAB_Activity_Uji_Sampel')->insertGetId([
                'Kode_Perusahaan' => '001', 'No_Po_Sampel' => $firstAnalysis['No_Po_Sampel'],
                'No_Fak_Sub_Po' => $firstAnalysis['No_Po_Multi_Sampel'], 'Jenis_Aktivitas' => 'save_submit_resampling', // Ubah jenis aktivitas
                'Keterangan' => $pengguna->Nama . ' Berhasil Mengirimkan Data Resampling', 'Id_User' => $pengguna->UserId,
                'Tanggal' => $tanggalSqlServer, 'Jam' => $jamSqlServer, 'Id_Jenis_Analisa' => $jenisAnalisaHeader
            ], 'Id_Log_Activity');
            $logChannel->info('--- [DEBUG] 6. Insert Log Utama Selesai', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            $logChannel->info('--- [DEBUG] 7. Mulai Loop Utama (Processing)', ['total_analyses' => count($request->analyses), 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            ...
            foreach ($request->analyses as $idx => $analysisData) {
                ...
                $idJenisAnalisaHash = $analysisData['Id_Jenis_Analisa'];
                $idMesin = $analysisData['id_mesin'];
                $currentNoPoMultiSampel = $analysisData['No_Po_Multi_Sampel']; 

                if ($existingResamplingMap->has($currentNoPoSampel)) {
                    $usedSubSamples = $existingResamplingMap->get($currentNoPoSampel);
                    
                    if ($usedSubSamples->has($currentNoPoMultiSampel)) {
                        // DUPLIKAT DITEMUKAN!
                        DB::rollBack();
                        $logChannel->warning('Resampling Ditolak: Duplikat No_Fak_Sub_Po', [
                            'userId' => $userId,
                            'no_po_sampel' => $currentNoPoSampel,
                            'no_fak_sub_po' => $currentNoPoMultiSampel,
                            'message' => 'Upaya untuk menggunakan No_Fak_Sub_Po yang sama dengan data resampling sebelumnya.'
                        ]);
                        return response()->json([
                            'success' => false, 
                            'status' => 409, // 409 Conflict
                            'message' => "Data resampling ditolak. Nomor Sub Sampel '$currentNoPoMultiSampel' sudah pernah digunakan untuk Resampling pada Sampel '$currentNoPoSampel'."
                        ], 409);
                    }
                }

                $idResamplingHash = $analysisData['Id_Resampling'];
                $tahapanKeFromRequest = (int) $analysisData['Tahapan_Ke'];
                
                $decodedResampling = Hashids::connection('custom')->decode($idResamplingHash);
                $idResampling = $decodedResampling[0] ?? null;
                
                if (!$idResampling) {
                    DB::rollBack(); 
                    $logChannel->error('ID Resampling tidak valid dalam loop', ['hash' => $idResamplingHash]);
                    return response()->json(['success' => false, 'status' => 400, 'message' => 'ID Resampling tidak valid.'], 400);
                }
                $resamplingIdsToUpdate[] = $idResampling; 
                $idDecodedJA = Hashids::connection('custom')->decode($idJenisAnalisaHash);
                $jenisAnalisaId = $idDecodedJA[0] ?? null;

                if (!$jenisAnalisaId) {
                    DB::rollBack();
                    $logChannel->error('ID Jenis Analisa tidak valid', ['hash' => $idJenisAnalisaHash]);
                    return response()->json(['success' => false, 'status' => 400, 'message' => 'ID Jenis Analisa tidak valid.'], 400);
                }
                ...
                if (!$isFlagKhusus) {
                    ...
                    if (!$isAllowed) {
                        DB::rollBack();
                        $logChannel->warning('Akses ditolak', ['userId' => $userId, 'jenisAnalisaId' => $jenisAnalisaId]);
                        return response()->json(['success' => false, 'status' => 403, 'message' => 'Akses ditolak untuk jenis analisa ini.'], 403);
                    }
                }
                ...
                // Tentukan Sumber Data & User ID
                $sumberData = null;
                $idUserUntukInsert = $userId;
                $isFromSementara = false;

                // Decode parameter QC IDs dari request
                $decodedParametersRequest = collect($analysisData['parameters'])->mapWithKeys(function ($param) {
                    $decoded = Hashids::connection('custom')->decode($param['Id_Quality_Control']);
                    $qcId = $decoded[0] ?? null;
                    if ($qcId) {
                        return [$qcId => ['Value_Parameter' => $param['Value_Parameter']]];
                    }
                    return [];
                })->filter();

                // Logika Penggabungan Data Sementara (Sama seperti v15)
                if ($noSementara && $sementaraHeaderMap->has($noSementara)) {
                    $dataSementara = $sementaraHeaderMap->get($noSementara);
                    $detailsSementara = $sementaraDetailMap->get($noSementara);

                    if ($dataSementara && $detailsSementara) {
                        $isFromSementara = true;
                        $idUserUntukInsert = $dataSementara->Id_User;
                        $sementaraToDelete[] = $noSementara;

                        $mergedParameters = $detailsSementara->map(function ($dbParam) use ($decodedParametersRequest) {
                            $qcId = $dbParam->Id_Quality_Control;
                            $finalValue = $dbParam->Value_Parameter;
                            if (is_null($finalValue) && $decodedParametersRequest->has($qcId)) {
                                $finalValue = $decodedParametersRequest->get($qcId)['Value_Parameter'];
                            }
                        ...
                        DB::rollBack();
                        $logChannel->error('Data sementara tidak konsisten', ['no_sementara' => $noSementara]);
                        return response()->json(['success' => false, 'status' => 500, 'message' => 'Data sementara tidak konsisten.'], 500);
                    }
                }
                if (!$sumberData) {
                ...
                $kodeBarang = $kodeBarangMap->get($sumberData->No_Po_Sampel);
                $formulasForJA = $formulaMap->get($sumberData->Id_Jenis_Analisa) ?? collect();
                $mesinData = $mesinMap->get($sumberData->Id_Mesin); 
                $parameterValuesForCalc = collect($sumberData->parameters)->pluck('Value_Parameter', 'Id_Quality_Control');
                $clientFormulaResults = collect($sumberData->formulas ?? [])->pluck('Hasil_Perhitungan', 'Rumus');

                foreach ($formulasForJA as $formula) {
                    $idPerhitungan = $formula->id;
                    $rawDbFormula = $formula->Rumus; 
                    ...
                    $isMesinFg = $mesinData && $mesinData->Flag_FG == 'Y';
                    $tahapanKeUntukInsert = $isMesinFg ? $tahapanKeFromRequest : 1;
                    ...
                    $payloadsUjiSampel[] = [
                        ...
                        "No_Fak_Sub_Po" => $sumberData->No_Po_Multi_Sampel,
                        "Status" => null, 
                        'Tahapan_Ke' => $tahapanKeUntukInsert, 
                        "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer,
                        "Id_User" => $idUserUntukInsert,
                        "No_Po_Sampel" => $sumberData->No_Po_Sampel,
                        "Range_Awal" => $rangeAwal, "Range_Akhir" => $rangeAkhir,
                    ...
                    $payloadsActivityHasilDetail[] = [
                        ...
                        "Id_Jenis_Analisa" => $sumberData->Id_Jenis_Analisa,
                        "Id_Perhitungan" => $idPerhitungan,
                        "Value_Baru" => $hasilFloat, "Value_Lama" => $hasilFloat,
                        "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer,
                        "Id_User" => $idUserUntukInsert, "Status_Submit" => "Submited",
                    ];
                } 
                ...
                foreach ($sumberData->parameters as $parameter) {
                    $paramValueFloat = $this->safeFloat($parameter['Value_Parameter']);
                    $qcIdNumeric = $parameter['Id_Quality_Control'];

                    $payloadsUjiSampelDetail[] = [
                        "Kode_Perusahaan" => "001", "No_Faktur_Uji_Sample" => $newNumber,
                        "Id_Quality_Control" => $qcIdNumeric,
                        "Value_Parameter" => $paramValueFloat,
                        "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer,
                        "Id_User" => $idUserUntukInsert,
                    ];
                    ...
                    $payloadsActivityParameterDetail[] = [
                        ...
                        "Id_Jenis_Analisa" => $sumberData->Id_Jenis_Analisa,
                        "Id_Quality_Control" => $qcIdNumeric,
                        "Value_Baru" => $paramValueFloat, "Value_Lama" => $paramValueFloat,
                        "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer,
                        "Id_User" => $idUserUntukInsert, "Status_Submit" => "Submited",
                    ];
                } 
            ...
            $uniqueResamplingIds = array_unique($resamplingIdsToUpdate);
            if (!empty($uniqueResamplingIds)) {
                DB::table('N_EMI_LAB_Uji_Sampel_Resampling_Log')
                    ->whereIn('Id_Resampling', $uniqueResamplingIds)
                    ->update(['Flag_Selesai_Resampling' => 'Y']);
            }
            $logChannel->info('--- [DEBUG] 8d. Bulk Update Resampling Log Selesai', ['count' => count($uniqueResamplingIds), 'elapsed' => number_format((microtime(true) - $bulkOpsStartTime) * 1000, 2) . ' ms']);


            $logChannel->info('--- [DEBUG] 8e. Selesai Bulk Operations', ['elapsed' => number_format((microtime(true) - $bulkOpsStartTime) * 1000, 2) . ' ms']);
            ...
            $logChannel->info('✅ PROSES RESAMPLING SELESAI (vOpt.2)', [
                'status' => 'Optimal (Full Optimize: Pre-fetch, Bulk Ops, Async Log, Resampling Logic, Anti-Dupe)',
                'executionTime' => number_format($executionTime, 4) . ' detik',
                'userId' => $userId,
                'totalAnalysisInRequest' => count($request->analyses),
                'totalRowsInsertedSynchronously' => $totalRowsInsertedSynchronously,
                'totalLogRowsQueued' => $totalLogRowsQueued,
                'totalResamplingLogsUpdated' => count($uniqueResamplingIds),
            ]);

            return response()->json([
                'success' => true, 'status' => 201,
                'message' => 'Data resampling berhasil diproses dan disimpan (Log detail diproses di background).',
                'results' => $results
            ], 201);
        } catch (\Exception $e) {
            DB::rollBack();
            $logChannel->info('--- [DEBUG] Exception Terjadi, Rollback', ['elapsed' => isset($startTime) ? number_format((microtime(true) - $startTime) * 1000, 2) . ' ms' : 'N/A']);
            $endTime = microtime(true);
            $executionTime = isset($startTime) ? ($endTime - $startTime) : -1;

            $logChannel->error('❌ GAGAL PROSES RESAMPLING (vOpt.2)', [
                'status' => 'Gagal (Exception)',
                'executionTime' => number_format($executionTime, 4) . ' detik',
                'userId' => $pengguna->UserId ?? 'unknown',
                'errorMessage' => $e->getMessage(),
            ...
            // Jangan tampilkan trace di produksi, tapi ok untuk debug
            return response()->json(['success' => false, 'status' => 500, 'message' => 'Terjadi Kesalahan Server: ' . $e->getMessage(), 'trace' => $e->getTraceAsString()], 500);
        } finally {
            if (isset($startTime)) {
                $logChannel->info('--- [DEBUG] PROSES SELESAI (finally)', ['total_elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            }
    ...
    public function storeMultiRumusSementara(Request $request)
    {
        $request->validate([
            ...
            'analyses.*.No_Sementara' => 'nullable|string',
            'analyses.*.formulas' => 'present|array',
            'analyses.*.is_multi_print' => 'nullable',
        ]);
        
        $logChannel = Log::channel('TrackingPerformersSementara'); 
        $startTime = microtime(true);
        $logChannel->info('--- [DEBUG] PROSES DIMULAI (v13 - Indexed Query) ---');
        ...
        try {
            ...
            $logChannel->info('--- [DEBUG] 2. Memulai Kueri Setup Tergabung (Time, User, LastNumber)...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $setupSql = "
                SELECT 
                    (SELECT dbo.Get_Date_Time()) as DateTimeNow,
                    (SELECT 1 FROM N_EMI_LAB_Users WHERE UserId = ?) as UserExists,
                    (SELECT TOP 1 No_Sementara FROM N_EMI_LAB_Uji_Sampel_Sementara WHERE No_Sementara LIKE ? ORDER BY No_Sementara DESC) as LastNoSementara
            ";
            $setupResult = DB::selectOne($setupSql, [$userId, $prefix . '-%']);

            if (empty($setupResult) || empty($setupResult->DateTimeNow)) {
                DB::rollBack();
                throw new \Exception('Gagal mengambil waktu server (DateTimeNow is null).');
            }
            $dt = $setupResult->DateTimeNow; 
            $tanggalSqlServer = date('Y-m-d', strtotime($dt)); 
            $jamSqlServer = date('H:i:s', strtotime($dt));
            ...
            if (empty($setupResult->UserExists)) {
                DB::rollBack(); 
                $logChannel->warning('--- [DEBUG] User Tidak Ditemukan (Setup)', ['userId' => $userId]);
                return response()->json(['success'=>false,'status'=>404,'message'=>'User tidak ditemukan'], 404);
            }
            
            $lastNumber = $setupResult->LastNoSementara ? (int) substr($setupResult->LastNoSementara, -4) : 0;
            $logChannel->info('--- [DEBUG] 3. Selesai Kueri Setup Tergabung', ['lastNumber' => $lastNumber, 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $allNoPoSampel = $request->collect('analyses')->pluck('No_Po_Sampel')->unique();
            $allJenisAnalisaHashes = $request->collect('analyses')->pluck('Id_Jenis_Analisa')->unique();
            $allJenisAnalisa = $allJenisAnalisaHashes->map(function ($hash) { if (empty($hash)) return null; $decoded = Hashids::connection('custom')->decode($hash); return $decoded[0] ?? null; })->filter()->unique();
            
            $flagKhususMap = DB::table('N_EMI_LAB_PO_Sampel')->whereIn('No_Sampel', $allNoPoSampel)->where('Flag_Khusus', 'Y')->pluck('No_Sampel')->flip();
            $logChannel->info('--- [DEBUG] 4. flagKhususMap Selesai', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            
            $allowedAnalisaMap = DB::table('N_EMI_LAB_Barang_Analisa')->where('Id_User', $userId)->whereIn('Id_Jenis_Analisa', $allJenisAnalisa)->pluck('Id_Jenis_Analisa')->flip();
            $logChannel->info('--- [DEBUG] 5. allowedAnalisaMap Selesai', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            $results = [];
            $payloadsUjiSampelSementara = []; $payloadsUjiSampelDetailSementara = [];
            $payloadsActivityHasilDetail = []; $payloadsActivityParameterDetail = []; 
            $totalUpdatesHasil = 0; $totalUpdatesParameter = 0; $totalAnalysisProcessed = count($request->analyses);
            $totalAnalysisBaru = 0; $totalAnalysisDiUpdate = 0;
            $payloadsBatchUpdateHasil = [];
            $payloadsBatchUpdateParameter = [];

            $noPoSampel = $request->analyses[0]['No_Po_Sampel']; $No_Po_Multi_Sampel = $request->analyses[0]['No_Po_Multi_Sampel'];
            $idDecodedHeader = Hashids::connection('custom')->decode($request->analyses[0]['Id_Jenis_Analisa']); $jenisAnalisaHeader = isset($idDecodedHeader[0]) ? $idDecodedHeader[0] : null;
            $idLogActivity = DB::table('N_EMI_LAB_Activity_Uji_Sampel')->insertGetId([
                'Kode_Perusahaan' => '001', 'No_Po_Sampel' => $noPoSampel, 'No_Fak_Sub_Po' => $No_Po_Multi_Sampel, 'Jenis_Aktivitas' => 'save_draft',
                'Keterangan' => $pengguna->Nama . ' Menyimpan Data Analisa Sebagai Draft', 'Id_User' => $pengguna->UserId, 'Tanggal' => $tanggalSqlServer,
                'Jam' => $jamSqlServer, 'Id_Jenis_Analisa' => $jenisAnalisaHeader
            ], 'Id_Log_Activity');
            $logChannel->info('--- [DEBUG] 6. Insert Log Utama Selesai', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            $existingQueryBuilder = DB::table('N_EMI_LAB_Uji_Sampel_Sementara'); $uniqueKeys = new Collection();
            foreach ($request->analyses as $analysis) {
                $idDecoded = Hashids::connection('custom')->decode($analysis['Id_Jenis_Analisa']); $jenisAnalisa = isset($idDecoded[0]) ? $idDecoded[0] : null;
                $key = $analysis['No_Po_Sampel'].'|'.$analysis['No_Sementara'].'|'.$analysis['No_Po_Multi_Sampel'].'|'.$jenisAnalisa;
                if (!$uniqueKeys->has($key)) { $existingQueryBuilder->orWhere(function($query) use ($analysis, $jenisAnalisa) { $query->where('No_Po_Sampel',$analysis['No_Po_Sampel'])->where('No_Sementara',$analysis['No_Sementara'])->where('No_Fak_Sub_Po',$analysis['No_Po_Multi_Sampel'])->where('Id_Jenis_Analisa',$jenisAnalisa);}); $uniqueKeys->put($key, true); }
            }
            $allExisting = $existingQueryBuilder->get(); 
            $logChannel->info('--- [DEBUG] 7. Pre-fetch existingMap Selesai', ['count' => count($allExisting), 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $existingMap = $allExisting->keyBy(function($item) { return $item->No_Po_Sampel.'|'.$item->No_Sementara.'|'.$item->No_Fak_Sub_Po.'|'.$item->Id_Jenis_Analisa; });

            $logChannel->info('--- [DEBUG] 8a. Memulai Pra-Fetch Data Update...', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $startTimePrefetch = microtime(true);
            $existingNoSementaraList = $existingMap->pluck('No_Sementara')->unique()->filter();

            $allParamsResults = collect();
            if ($existingNoSementaraList->isNotEmpty()) {
                $allParamsResults = DB::table('N_EMI_LAB_Uji_Sampel_Detail_Sementara')
                                    ->whereIn('No_Sementara', $existingNoSementaraList)
                                    ->get();
            }
            $allParamsMap = $allParamsResults->groupBy('No_Sementara')->map(function($items) {
                return $items->keyBy(function($param) {
                    return $param->Id_Quality_Control.'-'.$param->No_Urut;
                });
            });

            $allHasilMap = $allExisting->groupBy('No_Sementara')->map(function($items) {
                return $items->keyBy('No_Urut');
            });

            $logChannel->info(
                '--- [DEBUG] 8b. Selesai Pra-Fetch Data Update',
                [ 
                    'count' => $existingNoSementaraList->count(), 
                    'param_details_found' => $allParamsResults->count(), 
                    'hasil_found_in_memory' => $allExisting->count(), 
                    'elapsed' => number_format((microtime(true) - $startTimePrefetch) * 1000, 2) . ' ms'
                ]
            );

            $logChannel->info('--- [DEBUG] Mulai Loop Utama', ['total_analyses' => $totalAnalysisProcessed, 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            foreach ($request->analyses as $idx => $analysis) {
                if ($idx % 10 == 0 && $idx > 0) {  $logChannel->info('--- [DEBUG] Loop Utama Progress', ['item' => $idx, 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']); }
                $idDecoded = Hashids::connection('custom')->decode($analysis['Id_Jenis_Analisa']); $jenisAnalisa = isset($idDecoded[0]) ? $idDecoded[0] : null; $currentNoPoSampel = $analysis['No_Po_Sampel'];
                $isFlagKhusus = isset($flagKhususMap[$currentNoPoSampel]); if (!$isFlagKhusus) { /* cek allowedAnalisaMap */ $isAllowed = isset($allowedAnalisaMap[$jenisAnalisa]); if (!$isAllowed) { DB::rollBack(); $logChannel->warning('--- [DEBUG] Akses Ditolak, Rollback', ['userId' => $userId, 'jenisAnalisa' => $jenisAnalisa, 'noPo' => $currentNoPoSampel]); return response()->json(['success' => false, 'status' => 403, 'message' => "Akses ditolak"], 403); }}
                $lookupKey = $analysis['No_Po_Sampel'].'|'.$analysis['No_Sementara'].'|'.$analysis['No_Po_Multi_Sampel'].'|'.$jenisAnalisa; $existing = $existingMap->get($lookupKey); $allowInsert = true;

                if ($existing) {
                    $existingNoSementara = $existing->No_Sementara; $parameterCount = count($analysis['parameters']); $matched = 0;
                    
                    $existingParamsMap = $allParamsMap->get($existingNoSementara);
                    $allExistingHasil = $allHasilMap->get($existingNoSementara);

                    if (is_null($existingParamsMap) || is_null($allExistingHasil)) {
                        $logChannel->warning('--- [DEBUG] Data Pra-Fetch tidak ditemukan, skip update.', ['no_sementara' => $existingNoSementara]);
                        $allowInsert = false; 
                        continue; 
                    }
                    
                    $localPayloadActivityParameterDetail = [];
                    foreach ($analysis['parameters'] as $param) {
                        $idDecodedQc = Hashids::connection('custom')->decode($param['Id_Quality_Control']); $idDecodedNu = !empty($param['No_Urut']) ? Hashids::connection('custom')->decode($param['No_Urut']) : []; $idNu = isset($idDecodedNu[0]) ? $idDecodedNu[0] : null; $idQc = isset($idDecodedQc[0]) ? $idDecodedQc[0] : null; $key = $idQc.'-'.$idNu; 
                        $existingParam = $existingParamsMap->get($key);
                        
                        if ($existingParam) {
                            if (is_null($existingParam->Value_Parameter) && !is_null($param['Value_Parameter'])) {

                                $parameterValues = collect($analysis['parameters'])->pluck('Value_Parameter', 'Id_Quality_Control'); $calculatedResults = [];
                                foreach ($analysis['formulas'] as $formula) { $idDecodedNuFormula = !empty($formula['No_Urut']) ? Hashids::connection('custom')->decode($formula['No_Urut']) : []; $idNuFormula = isset($idDecodedNuFormula[0]) ? $idDecodedNuFormula[0] : null; $hasilPerhitungan = $this->calculateFormulaServerSideV2($formula['Rumus'], $parameterValues, $formula['Digit']); $calculatedResults[] = ['No_Urut' => $idNuFormula, 'Hasil_Perhitungan' => $hasilPerhitungan]; } 
                                
                                foreach ($calculatedResults as $result) {
                                    $hasilFloat = $this->safeFloat($result['Hasil_Perhitungan']); 
                                    $getValueHasilLama = $allExistingHasil->get($result['No_Urut']); 
                                    $valueLama = $getValueHasilLama ? $this->safeFloat($getValueHasilLama->Hasil) : null;

                                    $payloadsActivityHasilDetail[] = [ "Kode_Perusahaan" => "001", 'Id_Log_Activity_Sampel' => $idLogActivity, "No_Po_Sampel" => $analysis['No_Po_Sampel'], "No_Fak_Sub_Po" => $analysis['No_Po_Multi_Sampel'], "Id_Jenis_Analisa" => $jenisAnalisa, "Value_Baru" => $hasilFloat, "Value_Lama" => $valueLama, "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer, "Id_User" => $pengguna->UserId, "Status_Submit" => "Drafted", ];

                                    $payloadsBatchUpdateHasil[] = [
                                        'No_Sementara' => $existingNoSementara, 'No_Urut' => $result['No_Urut'], 'Hasil' => $hasilFloat,
                                        'Tanggal' => $tanggalSqlServer, 'Jam' => $jamSqlServer, 'Id_User' => $pengguna->UserId
                                    ];
                                    $totalUpdatesHasil++;
                                }
                                foreach ($analysis['parameters'] as $paramDetail) {
                                    $idDecodedNuDetail = !empty($paramDetail['No_Urut']) ? Hashids::connection('custom')->decode($paramDetail['No_Urut']) : []; $idNuTu = isset($idDecodedNuDetail[0]) ? $idDecodedNuDetail[0] : null; $idDecodedQcDetail = !empty($paramDetail['Id_Quality_Control']) ? Hashids::connection('custom')->decode($paramDetail['Id_Quality_Control']) : []; $idQcDetail = isset($idDecodedQcDetail[0]) ? $idDecodedQcDetail[0] : null; $keyDetail = $idQcDetail.'-'.$idNuTu; 
                                    $getValueParamLama = $existingParamsMap->get($keyDetail); 
                                    $valueParamLama = $getValueParamLama ? $this->safeFloat($getValueParamLama->Value_Parameter) : null;

                                    $localPayloadActivityParameterDetail[] = [ "Kode_Perusahaan" => "001", 'Id_Log_Activity_Sampel' => $idLogActivity, "No_Po_Sampel" => $analysis['No_Po_Sampel'], "No_Fak_Sub_Po" => $analysis['No_Po_Multi_Sampel'], "Id_Jenis_Analisa" => $jenisAnalisa, "Id_Quality_Control" => $idQcDetail, "Value_Baru" => $this->safeFloat($paramDetail['Value_Parameter']), "Value_Lama" => $valueParamLama, "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer, "Id_User" => $pengguna->UserId, "Status_Submit" => "Drafted", ];

                                    $payloadsBatchUpdateParameter[] = [
                                        'No_Sementara' => $existingNoSementara, 'Id_Quality_Control' => $idQcDetail, 'No_Urut' => $idNuTu,
                                        'Value_Parameter' => $this->safeFloat($paramDetail['Value_Parameter']), 'Tanggal' => $tanggalSqlServer,
                                        'Jam' => $jamSqlServer, 'Id_User' => $pengguna->UserId
                                    ];
                                    $totalUpdatesParameter++;
                                }
                                $payloadsActivityParameterDetail=array_merge($payloadsActivityParameterDetail, $localPayloadActivityParameterDetail); $totalAnalysisDiUpdate++; $allowInsert = false;
                            } elseif ((float) $existingParam->Value_Parameter === (float) $param['Value_Parameter']) { $matched++; }
                        } else { 
                            $logChannel->warning("--- [DEBUG] Param tidak ditemukan (update)", ['param' => $param, 'no_sementara' => $existingNoSementara, 'key_lookup' => $key]); 
                        }
                    }
                    if ($matched === $parameterCount) { $allowInsert = false; }
                } 
                ...
                $parameterValues = collect($analysis['parameters'])->pluck('Value_Parameter', 'Id_Quality_Control'); $calculatedResults = [];
                foreach ($analysis['formulas'] as $formula) { $hasilPerhitungan = $this->calculateFormulaServerSideV2($formula['Rumus'], $parameterValues, $formula['Digit']); $calculatedResults[] = ['Hasil_Perhitungan' => $hasilPerhitungan]; } 
                $localPayloadUjiSampeSementara = []; $localPayloadActivityUjiSampelHasil = [];
                foreach ($calculatedResults as $result) {
                    $hasilFloat = $this->safeFloat($result['Hasil_Perhitungan']);
                    $localPayloadUjiSampeSementara[] = [ 'Kode_Perusahaan' => '001', 'No_Sementara' => $newNumber, 'No_Po_Sampel' => $analysis['No_Po_Sampel'], 'No_Fak_Sub_Po' => $analysis['No_Po_Multi_Sampel'], 'Id_Jenis_Analisa' => $jenisAnalisa, 'Hasil' => $hasilFloat, 'Flag_Perhitungan' => 'Y', 'Flag_Multi_QrCode' => $analysis['is_multi_print'], 'Status' => null, 'Tanggal' => $tanggalSqlServer, 'Jam' => $jamSqlServer, 'Id_User' => $pengguna->UserId, ];
                    $localPayloadActivityUjiSampelHasil[] = [ "Kode_Perusahaan" => "001", 'Id_Log_Activity_Sampel' => $idLogActivity, "No_Po_Sampel" => $analysis['No_Po_Sampel'], "No_Fak_Sub_Po" => $analysis['No_Po_Multi_Sampel'], "Id_Jenis_Analisa" => $jenisAnalisa, "Value_Baru" => $hasilFloat, "Value_Lama" => $hasilFloat, "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer, "Id_User" => $pengguna->UserId, "Status_Submit" => "Drafted", ];
                }
                $localPayloadActivityUjiSampelDetailSementara = []; $localPayloadActivityUjiSampelDetail = [];
                foreach ($analysis['parameters'] as $param) {
                    $idDecodedQc = Hashids::connection('custom')->decode($param['Id_Quality_Control']); $idQc = isset($idDecodedQc[0]) ? $idDecodedQc[0] : null;
                    $localPayloadActivityUjiSampelDetailSementara[] = [ 'Kode_Perusahaan' => '001', 'No_Sementara' => $newNumber, 'Id_Quality_Control' => $idQc, 'Value_Parameter' => $this->safeFloat($param['Value_Parameter']), 'Tanggal' => $tanggalSqlServer, 'Jam' => $jamSqlServer, 'Id_User' => $pengguna->UserId, ];
                    $localPayloadActivityUjiSampelDetail[] = [ "Kode_Perusahaan" => "001", 'Id_Log_Activity_Sampel' => $idLogActivity, "No_Po_Sampel" => $analysis['No_Po_Sampel'], "No_Fak_Sub_Po" => $analysis['No_Po_Multi_Sampel'], "Id_Jenis_Analisa" => $jenisAnalisa, "Id_Quality_Control" => $idQc, "Value_Baru" => $this->safeFloat($param['Value_Parameter']), "Value_Lama" => $this->safeFloat($param['Value_Parameter']), "Tanggal" => $tanggalSqlServer, "Jam" => $jamSqlServer, "Id_User" => $pengguna->UserId, "Status_Submit" => "Drafted", ];
                }
                $payloadsUjiSampelSementara = array_merge($payloadsUjiSampelSementara, $localPayloadUjiSampeSementara); $payloadsUjiSampelDetailSementara = array_merge($payloadsUjiSampelDetailSementara, $localPayloadActivityUjiSampelDetailSementara);
                $payloadsActivityHasilDetail = array_merge($payloadsActivityHasilDetail, $localPayloadActivityUjiSampelHasil); $payloadsActivityParameterDetail = array_merge($payloadsActivityParameterDetail, $localPayloadActivityUjiSampelDetail);
                $totalAnalysisBaru++; $results[] = ['No_Sementara' => $newNumber];
            } 
            $logChannel->info('--- [DEBUG] Selesai Loop Utama', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            $logChannel->info('--- [DEBUG] Mulai Batch Updates (v9)', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $batchUpdateStartTime = microtime(true);

            if (!empty($payloadsBatchUpdateHasil)) {
                $updatesByNoSementara = collect($payloadsBatchUpdateHasil)->groupBy('No_Sementara');
                foreach ($updatesByNoSementara as $noSementara => $updates) {
                    $ids = $updates->pluck('No_Urut')->implode(',');
                    $hasilCases = ""; $bindings = [];
                    foreach ($updates as $update) {
                        $hasilCases .= " WHEN " . (int)$update['No_Urut'] . " THEN ?"; $bindings[] = $update['Hasil']; 
                    }
                    $bindings[] = $updates[0]['Tanggal']; $bindings[] = $updates[0]['Jam']; $bindings[] = $updates[0]['Id_User']; $bindings[] = $noSementara;
                    $sql = "UPDATE N_EMI_LAB_Uji_Sampel_Sementara SET Hasil = CASE No_Urut $hasilCases END, Tanggal = ?, Jam = ?, Id_User = ? WHERE No_Sementara = ? AND No_Urut IN ($ids)";
                    DB::update($sql, $bindings);
                }
            }
            if (!empty($payloadsBatchUpdateParameter)) {
                $updatesByNoSementara = collect($payloadsBatchUpdateParameter)->groupBy('No_Sementara');
                foreach ($updatesByNoSementara as $noSementara => $updates) {
                    $valueCases = ""; $whereInParts = []; $bindings = [];
                    foreach ($updates as $update) {
                        $qc = (int)$update['Id_Quality_Control']; $nu = (int)$update['No_Urut']; $val = $update['Value_Parameter'];
                        $valueCases .= " WHEN (Id_Quality_Control = $qc AND No_Urut = $nu) THEN ?"; $bindings[] = $val;
                        $whereInParts[] = "(Id_Quality_Control = $qc AND No_Urut = $nu)";
                    }
                    $whereInSql = implode(' OR ', $whereInParts);
                    $bindings[] = $updates[0]['Tanggal']; $bindings[] = $updates[0]['Jam']; $bindings[] = $updates[0]['Id_User']; $bindings[] = $noSementara;
                    $sql = "UPDATE N_EMI_LAB_Uji_Sampel_Detail_Sementara SET Value_Parameter = CASE $valueCases ELSE Value_Parameter END, Tanggal = ?, Jam = ?, Id_User = ? WHERE No_Sementara = ? AND ($whereInSql)";
                    DB::update($sql, $bindings);
                }
            }
            $logChannel->info('--- [DEBUG] 8c. Selesai Batch Updates (v9)', ['count_hasil' => count($payloadsBatchUpdateHasil), 'count_param' => count($payloadsBatchUpdateParameter), 'elapsed' => number_format((microtime(true) - $batchUpdateStartTime) * 1000, 2) . ' ms']);

            $logChannel->info('--- [DEBUG] Mulai Bulk Inserts (Data Utama)', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            if (!empty($payloadsUjiSampelSementara)) {
                DB::table('N_EMI_LAB_Uji_Sampel_Sementara')->insert($payloadsUjiSampelSementara);
            }
            $logChannel->info('--- [DEBUG] 9. Bulk Insert 1 (Uji Sampel) Selesai', ['count' => count($payloadsUjiSampelSementara), 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            if (!empty($payloadsUjiSampelDetailSementara)) {
                DB::table('N_EMI_LAB_Uji_Sampel_Detail_Sementara')->insert($payloadsUjiSampelDetailSementara);
            }
            $logChannel->info('--- [DEBUG] 10. Bulk Insert 2 (Detail Sampel) Selesai', ['count' => count($payloadsUjiSampelDetailSementara), 'elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $logChannel->info('--- [DEBUG] Selesai Bulk Inserts (Data Utama)', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $logChannel->info('--- [DEBUG] Mulai Commit (Data Utama)', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            DB::commit();
            $logChannel->info('--- [DEBUG] 13. Commit (Data Utama) Selesai', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);

            if (!empty($payloadsActivityHasilDetail) || !empty($payloadsActivityParameterDetail)) {
                LogLabActivityDetailsJob::dispatch($payloadsActivityHasilDetail, $payloadsActivityParameterDetail);
                $logChannel->info('--- [DEBUG] 14. Job LogLabActivityDetailsJob dispatched', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            }

            $endTime = microtime(true);
            $executionTime = $endTime - $startTime;
            $totalRowsInsertedSynchronously = count($payloadsUjiSampelSementara) + count($payloadsUjiSampelDetailSementara);
            $totalLogRowsQueued = count($payloadsActivityHasilDetail) + count($payloadsActivityParameterDetail);
            $totalRowsUpdated = $totalUpdatesHasil + $totalUpdatesParameter; 

            $logChannel->info('✅ PROSES STORE RUMUS SEMENTARA SELESAI (v13 - Indexed Query)', [ 
                'status' => 'Optimal (Indexed Setup, Batch Update, Pre-fetch)',
                'executionTime' => number_format($executionTime, 4) . ' detik',
                'userId' => $userId,
                'totalAnalysisInRequest' => $totalAnalysisProcessed,
                'totalAnalysisInserted' => $totalAnalysisBaru,
                'totalAnalysisUpdated' => $totalAnalysisDiUpdate,
                'totalRowsInsertedSynchronously' => $totalRowsInsertedSynchronously, 
                'totalLogRowsQueued' => $totalLogRowsQueued, 
            ...
            return response()->json([
                'success' => true, 'status' => 201,
                'message' => 'Data Berhasil Disimpan (Log diproses di background)',
                'result' => $results
            ], 201);
            ...
            $logChannel->info('--- [DEBUG] Exception Terjadi, Rollback', ['elapsed' => number_format((microtime(true) - $startTime) * 1000, 2) . ' ms']);
            $endTime = microtime(true); $executionTime = isset($startTime) ? ($endTime - $startTime) : -1;
            $logChannel->error('❌ GAGAL STORE RUMUS SEMENTARA (v13)', [
                'status' => 'Gagal (Exception)',
                'executionTime' => number_format($executionTime, 4) . ' detik',
                'userId' => $pengguna->UserId ?? 'unknown',
                'errorMessage' => $e->getMessage(),
