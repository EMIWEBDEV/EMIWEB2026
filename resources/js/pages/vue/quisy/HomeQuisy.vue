    ...
    methods: {
      ...
      `;
        ...
        async fetchMachines() {
            ...
            try {
                const response = await axios.get(
                    `/machines/${computerKey}/${noTransaksi}/${batchNumber}`
                );
        ...
        async loadSplitPO(poId) {
            ...
            try {
                const response = await axios.get(
                    `split-po/${poId}/${computer_keys}`
                );

                if (response.data.success && response.data.data) {
                    this.splitPOs = response.data.data;
                } else {
                    this.splitPOs = [];
        ...
        async loadBatches(splitPO) {
            ...
            try {
                const response = await axios.get(
                    `/batch-po/${id}/${computer_keys}`
                );
                if (
                    response.data.success &&
                    Array.isArray(response.data.data)
                ) {
                    splitPO.batches = response.data.data;
        ...
        async fetchPoList() {
            ...
            const computer_keys = localStorage.getItem("SSID_EVO");
            try {
                const response = await axios.get(`/api/v2/po/${computer_keys}`);
                ...
                if (response.status === 204) {
                    ...
                    this.poList = response.data.data;
        ...
        handleBatchClick(item) {
            ...
            this.fetchMachines();
            // if (this.isBatchSaved(item)) {
            //     Swal.fire({
            //         icon: "success",
            //         title: "Oops...",
            //         text: "Batch Sudah Selesai Masuk Ke Database Dan Berhasil Mohon Segera Selesaikan Yang Belum, Terimakasih!",
            //     });
            //     return;
            // } else {
            //     this.selectBatch(item);
        ...
        async saveDataToDatabase() {
            ...
            try {
                ...
                if (result.success && result.print_jobs && result.printer_url) {
                    const printJobs = result.print_jobs;
                    const printerUrl = result.printer_url;
                    ...
                    for (const job of printJobs) {
                        try {
                            ...
                            console.error(
                                "Gagal mengirim ke printer lokal:",
                                errPrint
                            );
                        }
                    }
