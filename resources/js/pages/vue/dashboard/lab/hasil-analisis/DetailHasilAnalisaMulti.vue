...
export default {
    ...
    methods: {
        ...
        processItems(items, template) {
            ...
            const processedData = Object.values(groupedByFaktur).map(
                (group) => {
                    ...
                    // --- LOGIKA PRIORITAS FLAG_LAYAK ---
                    // Cek seluruh item di dalam grup. Jika ada satu saja yang 'T',
                    // maka seluruh baris dianggap 'T'.
                    const groupFlagLayak = group.some(
                        (item) => item.Flag_Layak === "T"
                    )
                        ? "T"
                        : firstItemInGroup.Flag_Layak; // Jika tidak ada 'T', pakai flag item pertama.
                    // ------------------------------------
            ...
            // Menghitung rata-rata (tidak ada perubahan di bagian ini)
            ...
            // Hapus properti internal sebelum mengembalikan data
        ...
        getRowClass(row) {
            // Method ini tidak berubah, bekerja berdasarkan hasil dari processItems
            if (row.Flag_Layak === "T") {
                return "bg-danger text-white"; // Merah jika Tidak Layak
            }
            if (row.Flag_Layak === "Y") {
                return "bg-success text-white"; // Hijau jika Layak
            }
            return null; // Tidak ada warna jika flag tidak dikenali
        },
        ...
        async fetchHasilAnalisaByJenisAnalisa() {
            ...
            try {
                const [response, parameterResponse, currentDataTracking] =
                    await Promise.all([
                        axios.get(
                            `/api/v2/lab/hasil-analisa/multi/${this.id_jenis_analisa}/${this.no_po_sampel}/${this.flag_multi}/${this.no_fak_sub}`
                        ),
                        axios.get(
                            `/fetch/lab/lama/${this.id_jenis_analisa}/parameter-perhitungan-old`
                        ),
                        axios.get(
                            `/api/v1/tracking-detail/${this.no_po_sampel}/${this.no_fak_sub}/multi-print/${this.id_jenis_analisa}/analisa`
                        ),
                    ]);
