                        ...
                        <div v-else-if="listData.length > 0">
                            ...
                            >
                                <div class="d-flex align-items-center">
                                    <i :class="[summary.icon, 'fs-2 me-3']"></i>
                                    <div>
    ...
    computed: {
        timelineChartSeries() {
            ...
            this.listData.forEach((item) => {
                ...
                // 2. PENTING: Logika untuk periode 'tunggu' (idle) (garis merah)
                // Periode ini dimulai 1 hari SETELAH registrasi dan berakhir 1 hari SEBELUM pengujian.
                const tungguStart = new Date(regDate);
                tungguStart.setDate(regDate.getDate() + 1);
                tungguStart.setHours(0, 0, 0, 0); // Normalisasi ke awal hari

                const tungguEnd = new Date(testDate);
                tungguEnd.setDate(testDate.getDate() - 1);
                tungguEnd.setHours(23, 59, 59, 999); // Normalisasi ke akhir hari

                // Hanya tambahkan bar 'tunggu' jika ada jeda minimal 1 hari penuh.
                // Jika registrasi tgl 13 dan uji tgl 14, tidak ada hari tunggu.
                // Jika registrasi tgl 13 dan uji tgl 15, ada 1 hari tunggu (tgl 14).
        ...
        /**
         * Menyiapkan opsi konfigurasi untuk timeline chart (Gantt).
         */
        timelineChartOptions() {
            return {
                ...
                plotOptions: {
                    bar: {
                        horizontal: true, // Wajib true untuk Gantt chart
                        barHeight: "80%",
                ...
                xaxis: {
                    type: "datetime", // Sumbu X adalah waktu/tanggal
                    labels: {
                        datetimeUTC: false, // Tampilkan dalam zona waktu lokal
                        format: "dd MMM", // Format tanggal (contoh: 13 Sep)
                    },
                },
                yaxis: {
                    show: true,
                ...
                tooltip: {
                    x: {
                        format: "dd MMMM yyyy", // Format tanggal saat di-hover
                    },
                },
                legend: {
                    position: "top",
        ...
        // --- 👇 KODE BARU DAN YANG SUDAH DIPERBAIKI UNTUK GRAFIK DURASI ---
        durationChartSeries() {
            if (!this.listData || this.listData.length === 0) {
                return [];
            }
        ...
        durationChartOptions() {
            ...
            return {
                ...
                tooltip: {
                    y: {
                        // 💡 PERBAIKAN UTAMA ADA DI SINI dengan .bind(this)
                        formatter: function (
                            val,
                        ...
                        }.bind(this), // <-- Tambahkan .bind(this) di sini
                    },
                },
                title: {
        ...
        async fetchHasilAnalisaByJenisAnalisa() {
            ...
            try {
                const [response, parameterResponse] = await Promise.all([
                    axios.get(
                        `/api/v2/lab/verifikasi-analisa/multi/${this.Id_Jenis_Analisa}/${this.No_Sampel}/${this.No_Fak_Sub_Sampel}`
                    ),
                    axios.get(
