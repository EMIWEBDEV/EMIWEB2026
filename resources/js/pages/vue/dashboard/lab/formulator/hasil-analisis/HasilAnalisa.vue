                    ...
                    <div class="modal-body">
                        ...
                        <div v-show="currentStep === 3" class="step-content">
                            ...
                            <div class="alert alert-danger">
                                ...
                                Disarankan untuk mencetak laporan dalam format
                                <strong>PDF</strong>. Format PDF sudah stabil
                                dan sesuai standar. Sedangkan format
                                <strong>Excel</strong> masih dalam tahap
                                pengembangan dan kemungkinan belum sepenuhnya
                                kompatibel.
                            </div>
                        ...
                        >
                            ...
                            {{
                                isGenerateButtonDisabled
                                    ? "Harap centang 'Saya mengerti dan setuju'"
                                    : "Generate Laporan"
                            }}
                        </button>
                    </div>
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p class="text-muted mb-0 subtitle">
                                    ...
                                    Koleksi lengkap data trial laboratorium PT.
...
export default {
    ...
    watch: {
        ...
        selectedJenisPrint(val) {
            sessionStorage.setItem("printJenisCetak", val ?? "");
            if (val !== "psz") {
                this.pszConfirmation = false;
            }
        },
    },
    computed: {
        ...
        selectedAnalysisNames() {
            return (this.listDataJenisAnalisa || [])
                .filter((item) =>
                    (this.selectedAnalysis || []).includes(item.id)
                )
                .map((item) => item.Jenis_Analisa)
                .join(", ");
        },
        jenisCetakLabel() {
            if (this.selectedJenisPrint === "ringkas") return "Cetak Ringkas";
            if (this.selectedJenisPrint === "detail") return "Cetak Detail";
            return "-";
        },
        showPszOption() {
            if (!this.selectedAnalysis || this.selectedAnalysis.length === 0) {
                return false;
            }

            const selectedItems = this.listDataJenisAnalisa.filter((item) =>
                this.selectedAnalysis.includes(item.id)
            );
            return selectedItems.some(
                (item) => item.Kode_Analisa && item.Kode_Analisa.includes("PSZ")
            );
        },
        isPszReportSelected() {
        ...
        isGenerateButtonDisabled() {
            if (this.isPszReportSelected && !this.pszConfirmation) {
                return true;
            }
            return false;
        },
    ...
    methods: {
        ...
        async fetchHasilAnalisa() {
            ...
            try {
                const response = await this.retryFetch(
                    () => axios.get("/api/v1/formulator/hasil-trial/uji-trial"),
                    2,
                    500
                );
        ...
        async fetchListMesin() {
            this.loading.loadingListDataMesin = true;
            try {
                const response = await axios.get(
                    "/api/v1/lab/mesin/export-hasil-analisa"
                );
                if (response.status === 200 && response.data?.result) {
                    const mesinOptions = response.data.result.map((item) => ({
                ...
                this.loading.loadingListDataMesin = false;
            }
        },
        togglePrintModal() {
            if (!this.printModal) {
            ...
            const savedIsPerhitungan = sessionStorage.getItem(
                "printSelectedIsPerhitungan"
            );
            ...
            const savedJenisCetak = sessionStorage.getItem("printJenisCetak");

            this.currentStep = savedStep ? parseInt(savedStep) : 1;
            this.selectedAnalysis = savedAnalysis
                ? JSON.parse(savedAnalysis)
                : [];
            this.selectedIsPerhitungan = savedIsPerhitungan
                ? JSON.parse(savedIsPerhitungan)
                : [];
            this.startDate = savedStart || "";
            this.endDate = savedEnd || "";
            this.exportFormat = savedFormat || "excel";
            this.selectedJenisPrint = savedJenisCetak || ""; // tambahkan ini
        ...
        closePrintModal() {
            ...
            sessionStorage.removeItem("printSelectedIsPerhitungan");
            ...
            sessionStorage.removeItem("printJenisCetak");
        },
        nextStep() {
        ...
        validateCurrentStep() {
            if (this.currentStep === 1 && this.selectedAnalysis.length === 0) {
                this.showToast(
                    "Peringatan",
            ...
            if (this.currentStep === 3 && !this.selectedJenisPrint) {
                this.showToast(
                    "Peringatan",
                    "Pilih jenis cetakan terlebih dahulu",
                    "warning"
                );
                return false;
            }
        ...
        toggleAnalysisSelection(item) {
            const id = item.id;
            const perhitungan = item.Flag_Perhitungan;

            let newSelectedAnalysis = [...this.selectedAnalysis];
            let newSelectedIsPerhitungan = [...this.selectedIsPerhitungan];

            const index = newSelectedAnalysis.indexOf(id);
            if (index > -1) {
                newSelectedAnalysis.splice(index, 1);
                newSelectedIsPerhitungan.splice(index, 1);
            } else {
                newSelectedAnalysis.push(id);
                newSelectedIsPerhitungan.push(perhitungan);
            }

            this.selectedAnalysis = newSelectedAnalysis;
            this.selectedIsPerhitungan = newSelectedIsPerhitungan;
        },
        async generateReport() {
            ...
            try {
                ...
                if (this.selectedJenisPrint === "psz") {
                    const pszAnalysisData = this.listDataJenisAnalisa.find(
                        (item) =>
                            this.selectedAnalysis.includes(item.id) &&
                            item.Kode_Analisa.includes("PSZ")
                    );

                    if (!pszAnalysisData) {
                        Swal.fire(
                            "Peringatan",
                            "Analisa 'Particle Size' harus dipilih pada langkah pertama untuk membuat laporan ini.",
                            "warning"
                        );
                        return;
                    }
                    ...
                    payload = {
                        analysis: pszAnalysisData.id,
                        Flag_Perhitungan: pszAnalysisData.Flag_Perhitungan,
                    ...
                    urlLink =
                        exportFormat === "pdf"
                            ...
                            : isRingkas
                            ? "/api/v2/formulator/rekap-sampel/excell"
                            : "/api/v1/formulator/download-rekap/analisa";
                ...
                // (Sisa kode untuk memproses blob dan men-trigger download tidak perlu diubah)
