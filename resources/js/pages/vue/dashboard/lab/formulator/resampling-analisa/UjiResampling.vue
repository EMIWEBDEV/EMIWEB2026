<template>
    <div class="cleaning-system-container">
        ...
        <div class="system-header">
            <div class="header-content">
                <h1 class="system-title">
                    <i class="fas fa-vials"></i> Resampling & Reanalisa Sampel
                </h1>
        ...
        <div v-if="loading.detailSample" class="text-center loading-state">
        ...
        <div class="content-wrapper" v-else>
            <div class="search-panel">
                <div class="panel-header">
                    <h2><i class="fas fa-search"></i> Data Sampel</h2>
                </div>
                ...
                <div class="panel-body">
                    <div class="detail-grid">
                        <div class="detail-column">
                            ...
                            <div class="detail-item">
                                <span class="detail-label"
                                    >Analisa Yang Di Reanalisa</span
                                >
                                <span class="detail-value">{{
                                    nomorSampel.sampleDetails.resampling_info
                                        .Jenis_Analisa
                                }}</span>
                            </div>
                        </div>
                        <div class="detail-column">
                            ...
                            <div class="detail-item">
                                <span class="detail-label"
                                    >Nomor Sampel Uji Sebelumnya</span
                                >
                                <span class="detail-value">{{
                                    nomorSampel.sampleDetails.resampling_info
                    ...
                    <div class="notes-section" style="background: red">
                        <div class="notes-header">
                            <span class="notes-label text-white"
                                ><i class="fas fa-sticky-note"></i> Alasan
                                Reanalisa</span
                            >
                        </div>
                        <div class="notes-content text-white">
                            Sampel
                            <strong>{{
                                nomorSampel.sampleDetails.resampling_info
                                    .No_Sampel_Resampling_Origin
                            ...
                            <u class="ms-2"
                                >Saat Uji Tahap Sebelumnya Tidak Memenuhi
                                Standar Dari Nilai Yang Telah ditentukan</u
                            >
                            , Sehingga Dilakukan Reanalisa dengan nomor sampel
                            <strong>
                                {{
                                    nomorSampel.sampleDetails.resampling_info
                                        .No_Sampel_Resampling
                ...
                <div class="panel-body">
                    ...
                    >
                        <div
                            ...
                            class="analysis-card"
                            :class="{
                                'completed-card': item.is_done,
                                active: item.id === reactiveIdJenisAnalisa,
                            }"
                        >
            ...
            <div class="search-panel">
                <div class="panel-header">
                    <h2><i class="fas fa-search"></i> Cari Data</h2>
                </div>
                <div class="panel-body">
                    <div class="search-form">
                        <div class="form-group">
                            <label for="sampleNumber-multi" class="form-label"
            ...
            <div v-if="loading.detailTemplate">
                <div class="text-center my-4">
                    <span class="spinner-border"></span>
                    <p class="mt-2">Memuat formula...</p>
                </div>
            </div>

            <div
                v-else-if="
                    selectedTemplating && selectedTemplating.formula !== null
                "
            >
                <MultiRumus
                    :selectedTemplating="selectedTemplating"
                    :is_multi_print="
                        nomorSampel.sampleDetails?.is_multi_print ?? null
                    "
                    :Tahapan_Ke="
                        nomorSampel.sampleDetails.resampling_info?.Tahapan_Ke
                    "
                    :Id_Resampling="
                        nomorSampel.sampleDetails.resampling_info?.Id_Resampling
                    "
                    :no_ticket="nomorSampel.multiSampel?.no_ticket ?? null"
...
const MultiRumus = defineAsyncComponent(() =>
    import(
        "../../../../lab/formulator/perhitungan-backup/MultiRumusResamplingFormulator.vue"
    )
);

const NotRumus = defineAsyncComponent(() =>
    import(
        "../../../../lab/formulator/perhitungan-backup/NotRumusResamplingFormulator.vue"
    )
);
...
export default {
    props: {
        ...
        No_Fak_Sub_Sampel: [String, Number],
        Id_Jenis_Analisa: [String, Number],
        No_Resampling: [String, Number],
    },
    components: {
        DotLottieVue,
        MultiRumus,
    ...
    methods: {
        async initMounted() {
            ...
            const firstUndone = this.nomorSampel?.sampleDetails?.analisa?.find(
                (a) => !a.is_done
            );
            if (firstUndone) {
                this.handleClickLab(firstUndone.id);
                this.fetchNoMultiQrcode();
            }
        },

        handleClickLab: throttle(async function (idJenisAnalisa) {
        ...
        async fetchDetails() {
            ...
            try {
                ...
                sessionStorage.setItem("sampleNumber", this.No_Sampel);

                const response = await axios.get(
                    `/api/v1/lab/resampling/detail-data-sampel/${this.No_Sampel}/${this.No_Fak_Sub_Sampel}/${this.No_Resampling}/${this.Id_Jenis_Analisa}/formulator`
                );
        ...
        async fetchNoMultiQrcode() {
            ...
            try {
                const no_sampel =
                    this.nomorSampel.sampleDetails?.resampling_info
                        ?.No_Sampel_Resampling;
                ...
                const response = await axios.get(
                    `/api/v1/formulator/${no_sampel}/multi-print/${this.reactiveIdJenisAnalisa}/resampling`
                );
                ...
                if (
                    ...
                    console.log(
                        "Info: Data multi-print tidak ditemukan, beralih ke mode single/standar."
                    );
                    this.nomorSampel.multiSampel = null;
                } else {
                ...
                if (error.response && error.response.status === 404) {
                    console.log(
                        "Info: Endpoint multi-print 404, dianggap bukan multi-print."
                    );
                    return;
                }
