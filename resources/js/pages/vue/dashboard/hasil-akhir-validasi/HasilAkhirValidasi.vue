    ...
    <div class="fin-root">
        <div class="fin-topbar">
            <div class="fin-topbar-left">
                <div class="fin-topbar-icon-wrap"><i class="ri-checkbox-circle-line"></i></div>
                <div>
                    <span class="fin-topbar-title">Finalisasi Produksi</span>
                    <span class="fin-topbar-sub">Close sampel & rekam hasil akhir produksi</span>
                </div>
            </div>
            <div class="fin-topbar-right">
                <div v-if="pagination.total > 0" class="fin-stat-badge">
                    <span class="fin-stat-num">{{ pagination.total }}</span>
                    <span class="fin-stat-lbl">Menunggu</span>
                </div>
                <span class="fin-status-chip"><i class="ri-git-commit-line me-1"></i>Siap Finalisasi</span>
            </div>
        </div>
        ...
        <div class="fin-body">
            ...
            <div class="fin-left" :class="{ 'fin-hidden-mobile': detailVisible && isMobile }">
                <div class="fin-filter-bar">
                    ...
                    <div class="fin-filter-row">
                        <input type="date" class="fin-date-input" v-model="filters.startDate" @change="fetchList()" />
                        <span class="fin-sep">—</span>
                        <input type="date" class="fin-date-input" v-model="filters.endDate" @change="fetchList()" />
                        <select class="fin-select" v-model="filters.qrType" @change="fetchList()">
                            <option value="">Semua QR</option><option value="Y">Multi QR</option><option value="T">Single QR</option>
                        </select>
                ...
                <div class="fin-list">
                    ...
                    <div v-else-if="listData.length === 0" class="fin-empty-list">
                        ...
                        <button class="btn btn-sm btn-outline-primary" @click="resetFilters"><i class="ri-refresh-line me-1"></i>Reset</button>
                    </div>
                    <div v-else>
                        <div class="fin-checkall-bar">
                            <label class="fin-checkall-label">
                                <input type="checkbox" class="fin-checkall-cb" :checked="allCurrentPageChecked" :indeterminate.prop="someCurrentPageChecked && !allCurrentPageChecked" @change="toggleCheckAll" />
                            ...
                            <button @click="selectItem(item)" class="fin-item" :class="{ 'fin-item--active': isActive(item), 'fin-item--checked': isChecked(item) }">
                                ...
                                <div class="fin-item-body">
                                    <div class="fin-item-top">
                                        <span class="fin-item-title">{{ item.No_Po_Sampel }}</span>
                                        <span class="fin-badge fin-badge--primary">{{ item.Total_Jenis_Analisa }} Analisa</span>
                                    </div>
                ...
                <div v-if="selectedItems.length > 0" class="fin-bulk-bar">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button class="btn btn-sm btn-primary fw-semibold" @click="confirmBulk"><i class="ri-git-commit-line me-1"></i>Finalisasi Bulk</button>
                    </div>
                </div>
            ...
            <div class="fin-right" :class="{ 'fin-hidden-mobile': !detailVisible && isMobile }">
                <div v-if="isMobile && detailVisible" class="fin-mobile-back">
                ...
                <div v-if="!selectedItem" class="fin-detail-empty">
                    <div class="fin-detail-empty-inner">
                        <div class="fin-empty-icon-wrap"><i class="ri-git-commit-line"></i></div>
                        <h6>Pilih sampel untuk difinalisasi</h6>
                        <p>Klik item di daftar kiri untuk melihat detail analisa dan melakukan finalisasi.</p>
                    </div>
                </div>
                <template v-else>
                    <div class="fin-detail-header">
                        <div class="fin-dh-main">
                            <div class="fin-dh-icon"><i class="ri-git-commit-line"></i></div>
                            <div class="flex-grow-1 min-w-0">
                                ...
                                <div class="fin-dh-badges">
                                    ...
                                    <span class="fin-badge" :class="selectedItem.Flag_Multi_QrCode === 'Y' ? 'fin-badge--primary' : 'fin-badge--gray'">
                    ...
                    <div class="fin-detail-body">
                        <div v-if="loading.detail" class="fin-loading-state">
                        ...
                        <template v-else-if="activeTab === 'analisa'">
                            <div v-if="detailData.length === 0" class="fin-loading-state">
                                <i class="ri-inbox-2-line fs-1 text-muted"></i>
                                <p class="text-muted small mt-2">Tidak ada data analisa</p>
                            </div>
                            <div v-else>
                                <div v-for="section in detailSections" :key="section.group" class="fin-section">
                                    ...
                                    <button class="fin-section-hdr" @click="toggleSection(section.group)">
                                        <div class="fin-section-hdr-left">
                                            ...
                                            <div>
                                                <span class="fin-section-title">{{ section.label }}</span>
                                                <span class="fin-section-sub">{{ section.items.length }} jenis analisa</span>
                                            </div>
                                        </div>
                                    ...
                                    <!-- Section body: v-show (mounted immediately) -->
                                        ...
                                        <div v-for="analisa in section.items" :key="analisa.key" class="fin-analisa-panel">
                                            ...
                                            <button class="fin-analisa-hdr" :class="analisa.Flag_Layak === 'T' ? 'fin-analisa-hdr--danger' : 'fin-analisa-hdr--success'" @click="toggleAnalisa(analisa)">
                                                ...
                                                <div class="d-flex align-items-center gap-2 flex-shrink-0">
                                                    <span class="fin-badge" :class="analisa.Flag_Layak === 'T' ? 'fin-badge--danger' : 'fin-badge--success'">
                                                        <i :class="analisa.Flag_Layak === 'T' ? 'ri-close-circle-line' : 'ri-checkbox-circle-line'" class="me-1"></i>{{ analisa.Flag_Layak === 'T' ? 'Tidak Lolos' : 'Lolos' }}
                                                    </span>
                                                    <i class="fin-chevron ri-arrow-up-s-line" :class="{ 'collapsed': !isAnalisaOpen(analisa.key) }"></i>
                                                </div>
                                            </button>

                                            <!-- Table: v-show (mounted, no extra click needed after auto-open) -->
                                            <div v-show="isAnalisaOpen(analisa.key)" class="fin-analisa-table-wrap">
                                                <div v-if="loadingTable[analisa.key]" class="fin-table-loading">
                                                    <span class="spinner-border spinner-border-sm text-primary me-2"></span>
                                                    <span class="text-muted small">Memuat data...</span>
                                                </div>
                                                <div v-else-if="!tableRows[analisa.key] || tableRows[analisa.key].length === 0" class="text-center py-3 text-muted small">
                                                    <i class="ri-inbox-2-line me-1"></i>Tidak ada data
                                                </div>
                                                <div v-else>
                                                    <div class="table-responsive">
                                                        <table class="table table-sm table-bordered align-middle mb-0 fin-data-table">
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <template v-if="hasTemplateData(analisa.key)">
                                                                        <th v-for="param in getTemplate(analisa.key).parameter" :key="param.id_qc">
                                                            ...
                                                            <tbody>
                                                                ...
                                                                <!-- Rata-rata: only when calc + has formula columns -->
                                                    ...
                                                    <div v-if="tableHasFotos(analisa.key)" class="fin-foto-strip">
                                                        <button class="fin-foto-btn" @click="openFotoModal(analisa.key)">
                                                            <i class="ri-image-2-line me-1"></i>Lihat Foto ({{ fotoCount(analisa.key) }})
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <!-- TIMELINE TAB: Velzon Enterprise -->
                        <template v-else-if="activeTab === 'timeline'">
                            <div v-if="loading.timeline" class="fin-loading-state"><div class="spinner-border spinner-border-sm text-primary"></div><p class="text-muted small mt-2">Memuat...</p></div>
                                    ...
                                    <div v-for="(log, li) in auditLog" :key="li" class="fin-vtl-step" :class="getStepClass(log)">
                                        ...
                                        <div class="fin-vtl-body">
                                            <div class="fin-vtl-row1">
                                                <span class="fin-vtl-badge" :class="getStepBadgeClass(log)">{{ formatAksi(log.Jenis_Aksi) }}</span>
                                                <span v-if="log.Sub_Aksi" class="fin-vtl-sub" :class="log.Sub_Aksi==='TOLAK'?'text-danger':'text-success'">{{ log.Sub_Aksi }}</span>
                                            </div>
                                            <div class="fin-vtl-meta">
                    ...
                    <div class="fin-action-footer">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <button class="btn btn-primary fw-semibold px-4" @click="confirmSingle" :disabled="loading.submitting">
                                <span v-if="loading.submitting" class="spinner-border spinner-border-sm me-2"></span>
                                <i v-else class="ri-git-commit-line me-1"></i>Finalisasi Sampel
                            </button>
                        </div>
                    </div>
                </template>
        ...
        <div v-if="modal.show" class="fin-modal-backdrop" @click.self="modal.show = false">
            <div class="fin-modal">
                <div class="fin-modal-hdr"><i class="ri-git-commit-line me-2 fs-5"></i><div><div class="fin-modal-title">{{ modal.isBulk ? 'Bulk Finalisasi' : 'Konfirmasi Finalisasi' }}</div><div class="fin-modal-sub">{{ modal.isBulk ? selectedItems.length + ' sampel akan difinalisasi' : 'Sampel akan di-close dan dikunci' }}</div></div><button class="fin-modal-close" @click="modal.show = false"><i class="ri-close-line"></i></button></div>
                <div class="fin-modal-body">
                    <div v-if="!modal.isBulk && selectedItem">
                        <div class="fin-confirm-row"><span class="fin-confirm-lbl">Sampel</span><code class="fin-confirm-val">{{ selectedItem.No_Po_Sampel }}</code></div>
                        <div class="fin-confirm-row"><span class="fin-confirm-lbl">Barang</span><span class="fin-confirm-val">{{ selectedItem.Nama_Barang }}</span></div>
                        <div class="fin-confirm-row"><span class="fin-confirm-lbl">No. PO</span><span class="fin-confirm-val">{{ selectedItem.No_Po }}</span></div>
                        <div v-if="detailData.filter(d=>d.Flag_Layak==='T').length > 0" class="fin-modal-warn"><i class="ri-alert-line me-2"></i>{{ detailData.filter(d=>d.Flag_Layak==='T').length }} analisa tidak lolos. Status akhir TIDAK OK.</div>
                    </div>
                    <div v-else-if="modal.isBulk">
                        <p class="text-muted small mb-3">Sampel yang akan difinalisasi:</p>
                        <div v-for="(it, si) in selectedItems" :key="si" class="fin-bulk-row"><code class="fin-bulk-code">{{ it.No_Po_Sampel }}</code><span class="text-muted small">{{ it.Nama_Barang }}</span></div>
                    </div>
                </div>
                <div class="fin-modal-ftr">
                    <button class="btn btn-light" @click="modal.show = false">Batal</button>
                    <button class="btn btn-primary fw-semibold" @click="submitFinalisasi" :disabled="loading.submitting">
                        <span v-if="loading.submitting" class="spinner-border spinner-border-sm me-2"></span><i v-else class="ri-git-commit-line me-1"></i>Konfirmasi & Finalisasi
                    </button>
                </div>
            </div>
        </div>
        ...
        <!-- FOTO MODAL -->
        <div v-if="fotoModal.show" class="fin-modal-backdrop" @click.self="fotoModal.show = false">
            <div class="fin-modal fin-modal--wide">
                <div class="fin-modal-hdr fin-modal-hdr--neutral"><i class="ri-image-2-line me-2 fs-5"></i><div><div class="fin-modal-title">Foto Analisa</div><div class="fin-modal-sub">{{ fotoModal.currentIndex + 1 }} dari {{ fotoModal.photos.length }}</div></div><button class="fin-modal-close" @click="fotoModal.show = false"><i class="ri-close-line"></i></button></div>
                <div class="fin-modal-body text-center">
                    <img v-if="fotoModal.photos[fotoModal.currentIndex]" :src="fotoModal.photos[fotoModal.currentIndex]" class="img-fluid rounded" style="max-height:400px;object-fit:contain;" />
                    <div class="d-flex justify-content-center gap-2 mt-3" v-if="fotoModal.photos.length > 1">
                        <button class="btn btn-sm btn-outline-secondary" @click="fotoModal.currentIndex=Math.max(0,fotoModal.currentIndex-1)"><i class="ri-arrow-left-s-line"></i></button>
                        <button class="btn btn-sm btn-outline-secondary" @click="fotoModal.currentIndex=Math.min(fotoModal.photos.length-1,fotoModal.currentIndex+1)"><i class="ri-arrow-right-s-line"></i></button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        toggleSection(group) {
            const idx = this.openSections.indexOf(group);
            if (idx > -1) this.openSections.splice(idx, 1); else this.openSections.push(group);
        },
        toggleAnalisa(analisa) {
            const key = analisa.key;
            const idx = this.openAnalisas.indexOf(key);
            if (idx > -1) { this.openAnalisas.splice(idx, 1); return; }
            this.openAnalisas.push(key);
            if (!this.tableRows[key] && !this.loadingTable[key]) this.fetchAnalisaTable(analisa);
        },
        ...
        async fetchList() {
            ...
            try {
                ...
                const res = await axios.get("/api/v1/lab/validasi-hasil/akhir", { params });
                if (res.data?.success) {
                    this.listData = res.data.result?.data || res.data.result || [];
                    const meta = res.data.pagination || res.data.result?.pagination || {};
                    this.pagination = { page: meta.current_page||meta.page||1, totalPage: meta.total_pages||meta.totalPage||1, total: meta.total||meta.totalData||0, limit: meta.per_page||meta.limit||10 };
                } else { this.listData = []; }
            } catch { this.listData = []; } finally { this.loading.list = false; }
        },
        async selectItem(item) {
            this.selectedItem = item;
            this.activeTab = "analisa";
            this.auditLog = [];
            ...
            // Open ALL sections and analisa immediately
            ...
            // Load all tables in parallel (non-blocking)
            allItems.forEach(a => this.fetchAnalisaTable(a));
        },
        async fetchDetail(noSampel) {
            this.loading.detail = true; this.detailData = [];
            try { const res = await axios.get(`/api/v1/lab/validasi-akhir-close/uji-sampel/${noSampel}`); this.detailData = res.data?.result || []; }
            catch { this.detailData = []; } finally { this.loading.detail = false; }
        },
        async fetchAnalisaTable(analisa) {
            const key = analisa.key;
            const idJA = analisa.Id_Jenis_Analisa;
            const noSampel = this.selectedItem.No_Po_Sampel;
            this.loadingTable = { ...this.loadingTable, [key]: true };
            try {
                // Always call API: ensures Keterangan_Kriteria mapping and all rows (including PLT)
                ...
                const fotos = [];
                data.forEach(row => (row.foto_analisa || []).forEach(f => fotos.push(f)));
                if (fotos.length) this.tableRawFotos = { ...this.tableRawFotos, [key]: fotos };
            } catch (err) {
                console.error('fetchAnalisaTable error:', err);
                this.tableRows = { ...this.tableRows, [key]: [] };
        ...
        processItems(items, template) {
            ...
            const hasTemplate = tplParams > 0 || tplFormula > 0;

            // No template: show each row individually (simple non-calc, PLT without bindings)
            if (!hasTemplate) {
                return {
                    data: items.map(item => ({
                        No_Po: item.No_Po||'-', No_Split_Po: item.No_Split_Po||'-',
                        No_Faktur: item.No_Faktur||'-', Flag_Layak: item.Flag_Layak||'-',
                        No_Po_Sampel: item.No_Po_Sampel||'-', No_Fak_Sub_Po: item.No_Fak_Sub_Po||'-',
                        Tanggal: item.Tanggal_Pengujian||'-', Nama_Pembanding: item.Nama_Pembanding||null,
                        Hasil_Akhir_Analisa: this.formatHasil(item.Hasil_Akhir_Analisa),
                        parameters: [], results: [], foto_analisa: item.foto_analisa||[],
                    })),
                    formulaAverages: []
                };
            }
            ...
            const grouped = items.reduce((acc, item) => {
                const k = item.No_Faktur; if (!acc[k]) acc[k] = []; acc[k].push(item); return acc;
            }, {});
            ...
            const processedData = Object.values(grouped).map(group => {
                ...
                if (detailParams.length > 0) {
                    // Standard calculation: params from N_EMI_LAB_Uji_Sampel_Detail
                    parameterResults = detailParams.map(p => this.formatHasil(p.Hasil_Analisa));
                    // Only populate results when formula columns exist (prevents duplicate cells for non-calc/PLT)
                    finalResults = tplFormula > 0
                        ? group.map(item => ({ value: this.formatHasil(item.Hasil_Akhir_Analisa), Flag_Layak: item.Flag_Layak, pembulatan: item.Pembulatan ?? 4 }))
                        : [];
                } else if (group.length > 1 && tplParams > 0 && tplFormula === 0) {
                    // PLT: each row's Hasil maps to a parameter/criteria column
                    parameterResults = group.map(item => this.formatHasil(item.Hasil_Akhir_Analisa));
                    finalResults = [];
                } else {
                    // Single row or formula-only
                    parameterResults = [];
                    finalResults = group.map(item => ({ value: this.formatHasil(item.Hasil_Akhir_Analisa), Flag_Layak: item.Flag_Layak, pembulatan: item.Pembulatan ?? 4 }));
                }

                return {
                    No_Po: first.No_Po||'-', No_Split_Po: first.No_Split_Po||'-',
                    No_Faktur: first.No_Faktur||'-', Flag_Layak: first.Flag_Layak||'-',
                    No_Po_Sampel: first.No_Po_Sampel||'-', No_Fak_Sub_Po: first.No_Fak_Sub_Po||'-',
                    Tanggal: first.Tanggal_Pengujian||'-', Nama_Pembanding: first.Nama_Pembanding||null,
                    Hasil_Akhir_Analisa: this.formatHasil(first.Hasil_Akhir_Analisa),
                    parameters: parameterResults, results: finalResults, foto_analisa: first.foto_analisa||[],
                };
            });

            // Formula averages (only when formula columns exist)
            const formulaAverages = [];
            for (let i = 0; i < tplFormula; i++) {
                let total = 0, count = 0, dp = 4;
                processedData.forEach(row => { const r = row.results[i]; if (r && r.value !== '-') { const v = parseFloat(r.value); if (!isNaN(v)) { total += v; count++; if (r.pembulatan) dp = parseInt(r.pembulatan, 10); } } });
                formulaAverages.push(count > 0 ? (total/count).toFixed(dp) : '-');
            }
        ...
        formatHasil(val) {
            ...
            // Remove trailing .0 from whole floats: "5.0" → "5", "-223273928.0" → "-223273928"
            if (/^-?\d+\.0+$/.test(s)) return String(Math.trunc(parseFloat(s)));
            return s;
        },
        hasTemplateData(key) {
            const t = this.templates[key];
            return t && ((t.parameter?.length||0) > 0 || (t.formula?.length||0) > 0);
        },
        getTemplate(key) { return this.templates[key] || { parameter: [], formula: [] }; },
        getBaseColCount(analisa) {
            // # + No Transaksi + No Sampel + No PO + No Split Po + Tanggal = 6
            let c = 6;
            if (analisa.is_plt) c++;
            if (this.selectedItem?.Flag_Multi_QrCode === 'Y') c++;
            return c + (this.getTemplate(analisa.key).parameter?.length || 0);
        },
        ...
        openFotoModal(key) {
            ...
            this.fotoModal = { show:true, photos: fotos.map(f=>f.url||f.src||'').filter(Boolean), currentIndex:0 };
        },
        async loadTimeline() {
            if (this.auditLog.length > 0 || !this.selectedItem) return;
            this.loading.timeline = true;
            try { const res = await axios.get(`/api/v1/log-aksi/by-sampel/${this.selectedItem.No_Po_Sampel}`); this.auditLog = res.data?.result||[]; }
            catch { this.auditLog = []; } finally { this.loading.timeline = false; }
        },
        ...
        toggleCheckAll() {
            if (this.allCurrentPageChecked) { this.listData.forEach(item => { const idx = this.selectedItems.findIndex(i=>i.No_Po_Sampel===item.No_Po_Sampel); if (idx>-1) this.selectedItems.splice(idx,1); }); }
            else { this.listData.forEach(item => { if (!this.isChecked(item)) this.selectedItems.push(item); }); }
        },
        ...
        formatAksi(aksi) {
            const map={INPUT_ANALYZER:"Input Analyzer",VALIDASI_PRODUKSI:"Validasi Produksi",VALIDASI_TRIAL_PRODUKSI:"Validasi Trial",FINALISASI_PRODUKSI:"Finalisasi Produksi",FINALISASI_TRIAL_PRODUKSI:"Finalisasi Trial",VALIDASI_FORMULATOR:"Validasi Formulator",PRAFINALISASI_FORMULATOR:"Pra-Finalisasi",FINALISASI_FORMULATOR:"Finalisasi Formulator"};
...
/* === TOP BAR === */
.fin-topbar { display:flex; align-items:center; justify-content:space-between; padding:0 20px; height:54px; background:#fff; border-bottom:1px solid #e2e8f0; flex-shrink:0; gap:12px; box-shadow:0 1px 2px rgba(0,0,0,.06); }
.fin-topbar-left { display:flex; align-items:center; gap:10px; }
.fin-topbar-icon-wrap { width:34px; height:34px; border-radius:8px; background:linear-gradient(135deg,#405189,#2e3a64); display:flex; align-items:center; justify-content:center; color:#fff; font-size:.95rem; flex-shrink:0; }
...
/* === LEFT === */
.fin-left { width:360px; min-width:300px; display:flex; flex-direction:column; border-right:1px solid #e2e8f0; background:#fff; overflow:hidden; }
.fin-filter-bar { padding:10px 12px; border-bottom:1px solid #f1f5f9; flex-shrink:0; }
...
.fin-search-input:focus { border-color:#818cf8; box-shadow:0 0 0 3px rgba(129,140,248,.15); background:#fff; }
...
.fin-skeleton { height:66px; background:linear-gradient(90deg,#f1f5f9 25%,#e2e8f0 37%,#f1f5f9 63%); background-size:400% 100%; border-radius:7px; animation:vz-pulse 1.4s infinite; }
...
.fin-checkall-bar { display:flex; align-items:center; justify-content:space-between; padding:7px 12px; border-bottom:1px solid #f1f5f9; background:#f8fafc; }
.fin-checkall-label { display:flex; align-items:center; gap:7px; font-size:.78rem; font-weight:600; cursor:pointer; color:#374151; }
.fin-checkall-cb { cursor:pointer; accent-color:#405189; }
.fin-checkall-count { font-size:.73rem; color:#405189; font-weight:700; }
.fin-item-wrap { display:flex; align-items:stretch; border-bottom:1px solid #f1f5f9; }
.fin-item-checkbox { flex-shrink:0; margin:auto 9px; cursor:pointer; accent-color:#405189; }
...
.fin-item--active .fin-item-accent { background:#405189; }
...
.fin-dh-icon { width:38px; height:38px; border-radius:9px; background:linear-gradient(135deg,#405189,#2e3a64); color:#fff; display:flex; align-items:center; justify-content:center; font-size:1rem; flex-shrink:0; }
...
/* === SECTIONS === */
.fin-section { background:#fff; border-radius:9px; margin-bottom:7px; overflow:hidden; box-shadow:0 1px 3px rgba(0,0,0,.06); border:1px solid #e8ecf4; }
...
.fin-section-body { border-top:1px solid #f1f5f9; }

/* === ANALISA PANELS === */
.fin-analisa-panel { border-bottom:1px solid #f1f5f9; }
...
.fin-analisa-hdr:hover { background:#fafafa; }
...
.fin-analisa-table-wrap { padding-bottom:10px; background:#f9fafb; }
...
.fin-bulk-row { display:flex; align-items:center; gap:9px; padding:5px 9px; border-radius:5px; background:#f8fafc; margin-bottom:3px; }
.fin-bulk-code { font-size:.76rem; color:#405189; font-family:monospace; }
.fin-modal-ftr { display:flex; justify-content:flex-end; gap:9px; padding:11px 18px; background:#f8fafc; border-top:1px solid #f1f5f9; }
