    ...
    <div class="fin-root">
        <div class="fin-topbar">
            <div class="fin-topbar-left">
                <div class="fin-topbar-icon-wrap"><i class="ri-shield-star-line"></i></div>
                <div><span class="fin-topbar-title">Finalisasi Formulator</span><span class="fin-topbar-sub">Close sampel trial & rekam hasil validasi formulator</span></div>
            </div>
            <div class="fin-topbar-right">
                <div v-if="pagination.total > 0" class="fin-stat-badge"><span class="fin-stat-num">{{ pagination.total }}</span><span class="fin-stat-lbl">Menunggu</span></div>
                <span class="fin-status-chip"><i class="ri-shield-star-line me-1"></i>Siap Finalisasi</span>
            </div>
        </div>
        ...
        <div class="fin-body">
            ...
            <div class="fin-left" :class="{'fin-hidden-mobile':detailVisible&&isMobile}">
                <div class="fin-filter-bar">
                    <div class="fin-search-wrap"><i class="ri-search-line fin-search-icon"></i><input type="text" class="fin-search-input" placeholder="Cari No. Sampel, PO, Barang..." v-model="searchQuery" @input="debounceFetch" /><button v-if="searchQuery" class="fin-search-x" @click="searchQuery='';fetchList()"><i class="ri-close-line"></i></button></div>
                ...
                <div class="fin-list">
                    ...
                    <div v-else-if="listData.length===0" class="fin-empty-list"><i class="ri-inbox-2-line"></i><p>Tidak ada data siap finalisasi</p><button class="btn btn-sm fin-btn-form" @click="resetFilters"><i class="ri-refresh-line me-1"></i>Reset</button></div>
                    <div v-else>
                        <div v-for="(item,idx) in listData" :key="idx" class="fin-item-wrap">
                            <button @click="selectItem(item)" class="fin-item" :class="{'fin-item--active':isActive(item)}">
                                <div class="fin-item-accent" :class="{'fin-item-accent--active':isActive(item)}"></div>
                                <div class="fin-item-body">
                                    <div class="fin-item-top"><span class="fin-item-title">{{ item.No_Po_Sampel }}</span><span class="fin-badge fin-badge--form">Formulator</span></div>
                ...
                <div class="fin-list-footer" v-if="pagination.totalPage>1">
                    ...
                    <div class="fin-page-btns"><button class="fin-page-btn" :disabled="pagination.page===1" @click="changePage(pagination.page-1)"><i class="ri-arrow-left-s-line"></i></button><span class="fin-page-current">{{ pagination.page }} / {{ pagination.totalPage }}</span><button class="fin-page-btn" :disabled="pagination.page===pagination.totalPage" @click="changePage(pagination.page+1)"><i class="ri-arrow-right-s-line"></i></button></div>
                </div>
            </div>
            ...
            <div class="fin-right" :class="{'fin-hidden-mobile':!detailVisible&&isMobile}">
                ...
                <div v-if="!selectedItem" class="fin-detail-empty"><div class="fin-detail-empty-inner"><div class="fin-empty-icon-wrap"><i class="ri-shield-star-line"></i></div><h6>Pilih sampel formulator untuk difinalisasi</h6><p>Tampilkan detail analisa, info validator, dan lakukan finalisasi.</p></div></div>
                <template v-else>
                    <div class="fin-detail-header">
                        <div class="fin-dh-main">
                            <div class="fin-dh-icon"><i class="ri-shield-star-line"></i></div>
                            ...
                            <div v-else>
                                <div v-for="section in detailSections" :key="section.group" class="fin-section">
                                    <button class="fin-section-hdr" @click="toggleSection(section.group)">
                                        <div class="fin-section-hdr-left"><div class="fin-section-icon" :style="{background:section.bg}"><i :class="section.icon"></i></div><div><span class="fin-section-title">{{ section.label }}</span><span class="fin-section-sub">{{ section.items.length }} jenis analisa</span></div></div>
                                        ...
                                        <div v-for="analisa in section.items" :key="analisa.key" class="fin-analisa-panel">
                                            <button class="fin-analisa-hdr" :class="analisa.Flag_Layak==='T'?'fin-analisa-hdr--danger':'fin-analisa-hdr--success'" @click="toggleAnalisa(analisa)">
                                                <div class="fin-analisa-hdr-left">
                                                    ...
                                                    <div><span class="fin-analisa-hdr-title">{{ analisa.Jenis_Analisa }}</span><div class="fin-analisa-hdr-meta"><span class="fin-chip fin-chip--gray" style="font-size:.64rem;">{{ analisa.Kode_Analisa }}</span><span v-if="analisa.is_plt&&analisa.Nama_Pembanding" class="fin-chip fin-chip--cyan" style="font-size:.64rem;"><i class="ri-flask-line"></i>{{ analisa.Nama_Pembanding }}</span><span v-if="analisa.Flag_Perhitungan==='Y'" class="fin-chip fin-chip--purple" style="font-size:.64rem;"><i class="ri-calculator-line"></i>Perhitungan</span></div></div>
                                                </div>
                    ...
                    <div class="fin-action-footer">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <button class="btn fw-semibold px-4 fin-btn-form" @click="confirmSingle" :disabled="loading.submitting">
                                <span v-if="loading.submitting" class="spinner-border spinner-border-sm me-2"></span><i v-else class="ri-shield-star-line me-1"></i>Finalisasi Sampel
                            </button>
                        </div>
                    </div>
                </template>
        ...
        <div v-if="modal.show" class="fin-modal-backdrop" @click.self="modal.show=false">
            <div class="fin-modal">
                <div class="fin-modal-hdr fin-modal-hdr--form"><i class="ri-shield-star-line me-2 fs-5"></i><div><div class="fin-modal-title">Konfirmasi Finalisasi Formulator</div><div class="fin-modal-sub">Sampel akan dikunci secara permanen</div></div><button class="fin-modal-close" @click="modal.show=false"><i class="ri-close-line"></i></button></div>
                <div class="fin-modal-body" v-if="selectedItem">
                    ...
                    <div v-if="auditSummary.validators.length>0" class="fin-confirm-row"><span class="fin-confirm-lbl">Validator</span><span class="fin-confirm-val">{{ auditSummary.validators.map(v=>v.name).join(', ') }}</span></div>
...
export default {
    data() {
        return {
            ...
            auditSummary:{validators:[],prafinalisasi:[]},
    ...
    methods: {
        ...
        async fetchList(){
            ...
            try{
                ...
                const res=await axios.get("/api/v1/formualtor/finalisasi/hasil-uji-trial/current",{params});
        ...
        async selectItem(item){
            this.selectedItem=item;this.activeTab="analisa";this.auditLog=[];this.auditSummary={validators:[],prafinalisasi:[]};this.detailVisible=true;
            this.openSections=[];this.openAnalisas=[];this.templates={};this.tableRows={};this.tableAverages={};this.tableRawFotos={};
            await Promise.all([this.fetchDetail(item.No_Po_Sampel),this.fetchAuditLog(item.No_Po_Sampel)]);
        ...
        async fetchDetail(noSampel){this.loading.detail=true;this.detailData=[];try{const res=await axios.get(`/api/v1/finalisasi/formulator/detail-analisa/${noSampel}`);this.detailData=res.data?.result||[];}catch{this.detailData=[];}finally{this.loading.detail=false;}},
        async fetchAuditLog(noSampel){
            this.loading.timeline=true;
            try{const res=await axios.get(`/api/v1/log-aksi/by-sampel/${noSampel}`);this.auditLog=res.data?.result||[];this.buildAuditSummary();}
            catch{this.auditLog=[];}finally{this.loading.timeline=false;}
        },
        buildAuditSummary(){
            const validators=this.auditLog.filter(l=>l.Jenis_Aksi==='VALIDASI_FORMULATOR'&&l.Sub_Aksi==='SETUJU').map(l=>({name:l.Nama_User||l.Id_User,tanggal:this.formatDate(l.Tanggal)}));
            const prafinalisasi=this.auditLog.filter(l=>l.Jenis_Aksi==='PRAFINALISASI_FORMULATOR').map(l=>({name:l.Nama_User||l.Id_User,sub:l.Sub_Aksi,tanggal:this.formatDate(l.Tanggal)}));
            this.auditSummary={validators,prafinalisasi};
        },
        ensureTimeline(){if(this.auditLog.length===0&&this.selectedItem)this.fetchAuditLog(this.selectedItem.No_Po_Sampel);},
        async fetchAnalisaTable(analisa){
            ...
            try{
                // Formulator uses different API endpoints
                const [dataRes,templateRes]=await Promise.all([
                    axios.get(`/api/v1/formulator-final-keputusan/detail/${idJA}/single-qrcode/${noSampel}/final`).catch(()=>null),
                    axios.get(`/api/v1/formulator/uji-trial/${idJA}/parameter-perhitungan-old`).catch(()=>null),
                ]);
        ...
        confirmSingle(){this.modal.show=true;},
        async submitFinalisasi(){
            this.loading.submitting=true;
            const noSampel=this.selectedItem?.No_Po_Sampel||'';
            try{
                const res=await axios.post(`/api/v1/finalisasi/formulator/hasil-uji-trial/${noSampel}`);
                this.modal.show=false;
                if(res.data?.success){this.showToast("success","Sampel formulator berhasil difinalisasi");this.selectedItem=null;this.detailData=[];this.auditLog=[];this.detailVisible=false;this.fetchList();}
                else this.showErrorModal({title:"Finalisasi Gagal",noSampel,message:this.pickMessage(res.data,"Gagal finalisasi")});
        ...
        handleFinalisasiError(e){
            ...
            const data=res.data||{};
            const noSampel=data.detail?.No_Sampel||this.selectedItem?.No_Po_Sampel||"";
            if(res.status===419){
        ...
        getStepClass(log){if(log.Sub_Aksi==='TOLAK')return'vtl-rejected';if(log.Jenis_Aksi==='FINALISASI_FORMULATOR')return'vtl-final';if(log.Jenis_Aksi==='PRAFINALISASI_FORMULATOR')return'vtl-prafinal';return'vtl-done';},
        getStepIcon(log){if(log.Sub_Aksi==='TOLAK')return'ri-close-line';if(log.Jenis_Aksi==='INPUT_ANALYZER')return'ri-test-tube-line';if(log.Jenis_Aksi==='FINALISASI_FORMULATOR')return'ri-shield-star-line';if(log.Jenis_Aksi==='PRAFINALISASI_FORMULATOR')return'ri-check-double-line';return'ri-check-line';},
        getStepBadgeClass(log){if(log.Sub_Aksi==='TOLAK')return'vtl-badge--danger';if(log.Jenis_Aksi==='INPUT_ANALYZER')return'vtl-badge--info';if(log.Jenis_Aksi==='FINALISASI_FORMULATOR')return'vtl-badge--final';if(log.Jenis_Aksi==='PRAFINALISASI_FORMULATOR')return'vtl-badge--primary';return'vtl-badge--success';},
