                ...
                <div class="col-12 mt-3 content-area">
                    ...
                    <div class="row" v-else>
                        <template v-if="listData.length">
                            <hr class="mt-3" />
                            <div
                                class="d-flex justify-content-between align-items-start flex-wrap gap-2 w-100 mb-4"
                            >
                            ...
                            >
                                <a
                                    :href="`/finalisasi/formulator/hasil-uji-trial/by/detail/${item.No_Po_Sampel}/multi/${item.No_Fak_Sub_Po}/${item.Id_Jenis_Analisa}`"
                                >
...
export default {
    ...
    methods: {
        async fetchConfirmedUjiAnalisa() {
            ...
            try {
                const response = await axios.get(
                    `/api/v1/formualtor/finalisasi/hasil-uji-trial/${this.No_Sampel}`
                );
        ...
        `,
            ...
            try {
                await axios.post(
                    `/api/v1/finalisasi/formulator/hasil-uji-trial/${this.No_Sampel}`,
                    item
                );
                ...
                window.location.href = "/finalisasi/formulator/hasil-uji-trial";
            } catch (error) {
                console.error("Gagal mengirim data:", error);

                Swal.close(); // ✅ pastikan popup loading ditutup

                const res = error.response?.data;
                let message =
                    res?.message || "Terjadi kesalahan saat mengirim data!";
                let detailHtml = "";

                if (res?.detail) {
                    if (res.detail.No_Sampel) {
                        detailHtml += `
                <div style="border:1px solid red; padding:6px; margin-top:6px; border-radius:5px;">
                    <b>No Sampel:</b> <span style="color:red;font-weight:bold">${res.detail.No_Sampel}</span>
                </div>
            `;
                    }

                    if (res.detail.Analisa_Kurang) {
                        const analisaKurang = Object.values(
                            res.detail.Analisa_Kurang
                        ).join(", ");
                        detailHtml += `
                <div style="border:1px solid red; padding:6px; margin-top:6px; border-radius:5px;">
                    <b>Analisa belum lengkap:</b><br/>
                    <span style="color:red;font-weight:bold">${analisaKurang}</span>
                </div>
            `;
                    }

                    if (res.detail.Analisa_Belum_Validasi) {
                        const analisaBelumValid = Object.values(
                            res.detail.Analisa_Belum_Validasi
                        ).join(", ");
                        detailHtml += `
                <div style="border:1px solid orange; padding:6px; margin-top:6px; border-radius:5px;">
                    <b>Analisa belum divalidasi:</b><br/>
                    <span style="color:orange;font-weight:bold">${analisaBelumValid}</span>
                </div>
            `;
                    }
                    ...
                    detailHtml += `
            <div style="border:1px solid #666; padding:10px; margin-top:10px; border-radius:5px; background:#f9f9f9; color:#333;">
                <b>📢 Pemberitahuan:</b><br/>
                Ketentuan ini sudah terdaftar di <b>barang uji laboratorium</b> bahwa 
                analisa tersebut wajib diinput. <br/>
                Segera beritahu <b>Tim Analyzer</b> untuk melengkapi data analisa yang belum ada.
            </div>
        `;
                }
