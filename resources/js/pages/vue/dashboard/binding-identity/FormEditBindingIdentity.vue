...
export default {
        ...
        async submitFormToDatabase() {
            ...
            try {
                ...
                const response = await axios.put(
                    `/biding-identity/update/${this.getData.id}`,
                    payload,
                    {
                        headers: {
    ...
    watch: {
        computerIdentityList(newList) {
            if (newList.length && this.getData?.Id_Identity) {
                this.selectedOptionIdentity =
                    newList.find(
                        (item) => item.value == this.getData.Id_Identity
                    ) || null;
            }
        },
        mesinList(newList) {
            if (newList.length && this.getData?.Id_Mesin) {
                this.selectedOptionMesin =
                    newList.find(
                        (item) => item.value == this.getData.Id_Mesin
                    ) || null;
            }
        },
    },
...
export default {
    ...
    watch: {
        computerIdentityList(newVal) {
            if (newVal.length && this.getData?.Id_Identity) {
                const identity = newVal.find(
                    (item) => item.value == this.getData.Id_Identity
                );
                this.selectedOptionIdentity = identity || null;
            }
        },
        mesinList(newVal) {
            if (newVal.length && this.getData?.Id_Mesin) {
                const mesin = newVal.find(
                    (item) => item.value == this.getData.Id_Mesin
                );
                this.selectedOptionMesin = mesin || null;
            }
        },
    },
    ...
    mounted() {
        this.fetchComputerIdentityList();
        this.fetchMesinList();
        if (this.getData?.Id_Identity) {
            this.selectedOptionIdentity =
                this.computerIdentityList.find(
                    (item) => item.value == this.getData.Id_Identity
                ) || null;
        }

        if (this.getData?.Id_Mesin) {
            this.selectedOptionMesin =
                this.mesinList.find(
                    (item) => item.value == this.getData.Id_Mesin
                ) || null;
