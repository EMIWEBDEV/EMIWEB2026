        ...
        <div class="card shadow-sm border-0">
            <div class="card-body p-3 p-md-4 p-lg-5">
                ...
                <div class="col-12">
                    ...
                    <form action="{{ route('bindingjenisanalisa.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="Id_Jenis_Analisa" class="form-label fw-semibold">Jenis Analisa <span
                                    class="text-danger">*</span></label>
                            <select
                                class="js-example-basic-single form-control @error('Id_Jenis_Analisa') is-invalid shake @enderror"
                                name="Id_Jenis_Analisa" id="Id_Jenis_Analisa" data-placeholder="-- Pilih Jenis Analisis --">
                                <option value=""></option>
                                @foreach ($jenisAnalisa as $i)
                                    <option value="{{ $i->id }}"
                                        {{ old('Id_Jenis_Analisa') == $i->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            <label for="Id_Quality_Control" class="form-label fw-semibold">Parameter Quality <span
                                    class="text-danger">*</span></label>
                            <select
                                class="js-example-basic-single form-control @error('Id_Quality_Control') is-invalid shake @enderror"
                                name="Id_Quality_Control" id="Id_Quality_Control"
                                data-placeholder="-- Pilih Parameter Quality --">
                                <option value=""></option>
                                @foreach ($qualityControl as $i)
                                    <option value="{{ $i->Id_QC_Formula }}"
                                        {{ old('Id_Quality_Control') == $i->Id_QC_Formula ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            <label for="mesin_id" class="form-label fw-semibold">Keterangan</label>
