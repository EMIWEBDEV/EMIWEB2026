...
<head>
    ...
    <style>
        ...
        .main-table thead th {
            background-color: #D9EAD3; /* Warna header dari Excel */
            font-weight: bold;
...
<body>
   ...
   <main>
     <table class="main-table">
            ...
            @forelse ($collection as $item)
                <tr>
                    @foreach ($item as $key => $value)
                        {{-- numeric hanya untuk angka hasil parameter/rumus --}}
        ...
        {{-- Rata-rata (jika ada perhitungan) --}}
        @if ($apakahPerhitungan && !$collection->isEmpty())
            <tfoot>
                <tr>
                    @php
                        $colspan = count($headings) - $rumusCount;
                    @endphp
                    <td colspan="{{ $colspan }}" class="rata-rata-label">Rata-Rata</td>
